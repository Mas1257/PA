...
// @description  PA is your personal workspace for Amazon operations—bringing everything you need, every shift, into one place.
// @author       @zarkarma
// @match        *://*/*
// @grant        GM_addStyle
...
(function () {
    'use strict';
    if (window.top !== window.self) return; // Prevent running in iframes
    if (window.__PA_SCRIPT_INJECTED) return; // Fix for SPA double-initialization
    window.__PA_SCRIPT_INJECTED = true;

    class UIController {
        constructor() {
            this.panel = {
                element: null,
                show: () => { if (this.panel.element) this.panel.element.style.display = 'flex'; },
                hide: () => { if (this.panel.element) this.panel.element.style.display = 'none'; },
                isVisible: () => this.panel.element && this.panel.element.style.display !== 'none',
                append: (child) => { if (this.panel.element) this.panel.element.appendChild(child); },
                contains: (child) => this.panel.element ? this.panel.element.contains(child) : false,
                querySelector: (sel) => this.panel.element ? this.panel.element.querySelector(sel) : null,
                querySelectorAll: (sel) => this.panel.element ? this.panel.element.querySelectorAll(sel) : [],
                insertBefore: (newNode, refNode) => { if (this.panel.element) this.panel.element.insertBefore(newNode, refNode); },
                addEventListener: (type, listener, options) => { if (this.panel.element) this.panel.element.addEventListener(type, listener, options); }
            };
            this.formWrapper = { element: null };
            this.flashMessage = { element: null };
            this.panelAutoCloseTimer = null;
            this.modalAutoCloseTimer = null;
            this.panelHovering = false;
            this.modalHovering = false;
    ...
    // --- UNIFIED FOLDER UI ---
    UI.styleFolderMenuIcon = function (menuIcon, isSubFolder = false) {
    ...
    UI.createToolbarButton = function (text, icon, onClick) {
        const btn = document.createElement('button');
        btn.className = 'bm-button bm-toolbar-btn';
        ...
        btn.addEventListener('mouseenter', () => btn.style.backgroundColor = '#eeeeee');
        btn.addEventListener('mouseleave', () => btn.style.backgroundColor = '#f5f5f5');
        btn.addEventListener('click', onClick);
        return btn;
    };

    UI.createFolderIconSVG = function (hasContent, isSubFolder) {
        const fill1 = isSubFolder ? '#5b8def' : '#F6C358';
        const fill2 = isSubFolder ? '#7ba7f5' : '#FCD462';
        const paper = hasContent ?
            ...
            ``; // when empty, no paper rects

        const icon = document.createElement('div');
        icon.className = 'bm-folder-icon';
        icon.innerHTML = `<svg width="36" height="36" viewBox="0 0 491.52 491.52" xmlns="http://www.w3.org/2000/svg">
            <g>
            ...
            ${paper}
            <path style="fill:${fill2};" d="M474.806,216.429H16.714c-10.557,0-17.956,8.348-16.541,18.538l27.158,195.639c1.107,7.974,9.46,14.379,18.667,14.379h399.523c9.207,0,17.56-6.405,18.667-14.379l27.158-195.639C492.761,224.777,485.362,216.429,474.806,216.429z"/>
            </g>
            </svg>`;
        return icon;
    };

    UI.renderFolderGrid = function (folders, container, onOpen, onMenu) {
        const grid = document.createElement('div');
        Object.assign(grid.style, { display: 'flex', flexWrap: 'wrap', gap: '15px', width: '100%' });

        folders.forEach(folder => {
            const wrapper = document.createElement('div');
            wrapper.className = 'bm-folder-icon-wrapper';
            wrapper.setAttribute('data-folder', folder.name);
            wrapper.style.position = 'relative';

            wrapper.appendChild(UI.createFolderIconSVG(folder.hasContent, false));
    ...
    UI.renderSubFolderHeader = function (container, activeFolder, onBack, onAdd) {
        ...
        breadcrumb.className = 'bm-header folder-name';
        breadcrumb.style.margin = '0';
        breadcrumb.textContent = activeFolder;
        leftGroup.appendChild(breadcrumb);
        row.appendChild(leftGroup);

        const addSubBtn = document.createElement('button');
        addSubBtn.className = 'bm-button';
        addSubBtn.textContent = '+ Folder';
        addSubBtn.addEventListener('click', onAdd);
        row.appendChild(addSubBtn);

        container.appendChild(row);
    };

    UI.renderSubFolderGrid = function (subs, container, onOpen, onMenu) {
        if (!subs || subs.length === 0) return;
        const subGrid = document.createElement('div');
        Object.assign(subGrid.style, { display: 'flex', flexWrap: 'wrap', gap: '10px', width: '100%', marginBottom: '10px' });
        subs.forEach(sf => {
            const wrapper = document.createElement('div');
            wrapper.className = 'bm-folder-icon-wrapper';
            wrapper.title = sf.name;

            wrapper.appendChild(UI.createFolderIconSVG(sf.hasContent, true));
            ...
            wrapper.addEventListener('dblclick', () => onOpen(sf.name));

            const menuIcon = document.createElement('div');
            UI.styleFolderMenuIcon(menuIcon, true);
            menuIcon.addEventListener('click', e => {
                e.stopPropagation();
                closeAllContextMenus();
                const menu = buildContextMenu('bm-folder-menu-open');
                const closeMenu = openContextMenuAtEvent(menu, e, menuIcon, () => { });
                onMenu(sf, menu, closeMenu);
            });
            wrapper.appendChild(menuIcon);
    ...
    // --- END UNIFIED FOLDER UI ---

    class FolderManager {
        constructor() {
            this.barcode = { active: null, activeSub: null };
            this.bookmark = { active: null, activeSub: null };
            this.note = { active: null, activeSub: null };
        }
    }
    const Folders = new FolderManager();
    ...
    const STORAGE_KEYS = Object.freeze({
        ...
        NOTE_SUBFOLDERS: 'bm_note_subfolders',
    ...
    // --- Storage Engine (ES-001) ---
    // Low-level storage backend abstraction. Provides init, get, set,
    // remove, and emitChange as the only raw I/O primitives.
    // StorageService delegates all reads and writes to this layer,
    // keeping business-logic concerns (cache management, cross-tab
    // sync, invalidation) strictly inside StorageService.
    // To swap backends (e.g. for testing or cloud storage), replace
    // only this IIFE; no other code needs to change.
    const StorageEngine = (() => {
        /**
         * Initialize the storage engine.
         * No-op for the default GM/localStorage backend; reserved for
         * future backend setup (e.g. opening an IndexedDB connection or
         * authenticating with a remote service).
         */
        function init() {
            // No-op for current GM+localStorage backend.
        }
         ...
         *
         * Read priority:
         *   1. GM_getValue (cross-tab, survives page refreshes in Tampermonkey)
         *   2. localStorage (same-tab fallback; kept in sync on every write)
         *   3. fallback  (caller-supplied default)
         *
         * When GM returns a value that is not yet mirrored in localStorage
         * (e.g. after a cross-tab write), this method back-fills localStorage
         * so subsequent reads in the same tab are fast.
         * Conversely, when only localStorage has a value (GM returned nothing),
         * the value is pushed into GM so the next cross-tab read finds it.
         *
         * @param {string} key
         * @param {*} fallback
         * @returns {*}
        ...
        function get(key, fallback = null) {
            ...
            if (typeof GM_getValue === "function") {
                try {
                    ...
                    // Async GM (Promise-based) — use localStorage cache while waiting.
                    if (val && typeof val.then === 'function') {
                        return cached !== undefined ? cached : fallback;
                    }
                    if (val !== undefined) {
                        // Back-fill localStorage so same-tab reads stay fast.
                        try {
                            localStorage.setItem(key, JSON.stringify(val));
                        } catch { }
                        return val;
                    }
                    // GM returned nothing; push localStorage value into GM.
        ...
        /**
         * Write a value to persistent storage (GM + localStorage) and
         * broadcast a 'pa:storage-changed' CustomEvent so in-page
         * listeners (workspace auto-snapshot, cross-tab sync) can react
         * without polling.
         *
         * @param {string} key
         * @param {*} value
        ...
        /**
         * Delete a key from persistent storage and broadcast a change event.
         * GM does not have a native delete API, so the key is set to
         * `undefined` which causes GM_getValue to return its default on the
         * next read; the localStorage entry is fully removed.
         *
         * @param {string} key
         */
        function remove(key) {
        ...
        /**
         * Dispatch a 'pa:storage-changed' CustomEvent on the window so that
         * any in-page listener (e.g. WorkspaceService auto-snapshot tracking,
         * cross-tab sync helpers) is notified of the change.
         *
         * This is intentionally a fire-and-forget; listeners must not throw.
         *
         * @param {string} key
         */
        ...
        return Object.freeze({ init, get, set, remove, emitChange });
    })();

    // Initialise the engine immediately so the first gmGet / gmSet call is safe.
    StorageEngine.init();

    // --- Storage Service ---
    const StorageService = (() => {
        // Delegate raw I/O to StorageEngine; StorageService owns only cache
        // management, invalidation, and cross-tab synchronisation logic.
        function gmGet(key, fallback = null) {
            return StorageEngine.get(key, fallback);
        }
    ...
    * Future TODOs intentionally not implemented here:
    ...
    /* moved UI.switchTab to UI */
    /* moved UI.registerTab to UI */
    ...
    function scheduleQrPreviewPrefetch() {
        if (!UI.panel.element || !UI.panel.isVisible()) return;
    ...
    // refresh the current UI.panel.element after folder changes.
    // ============================================================
    ...
    const FolderDataService = (() => {
        ...
        function normalizeFolderDestination(preferredFolder = 'Default', preferredSubFolder = '') {
            return {
                folder: preferredFolder || Folders.barcode.active || 'Default',
        ...
        async function deleteFolder(folderName) {
            ...
            StorageService.gmSet(STORAGE_KEYS.FOLDERS, folders);
            setFoldersCache(folders);
            // Delete all nested folders belonging to this folder
            ...
            if (Folders.barcode.active === folderName) { Folders.barcode.active = null; Folders.barcode.activeSub = null; }
            renderFolders();
        ...
        async function renameFolder(oldName, newName) {
            ...
            StorageService.gmSet(STORAGE_KEYS.FOLDERS, folders);
            setFoldersCache(folders);
            // Update nested folder parent references
            let subFolders = getAllSubFolders();
            ...
            StorageService.gmSet(STORAGE_KEYS.BARCODES, barcodes);
            setBarcodesCache(barcodes);
            if (Folders.barcode.active === oldName) Folders.barcode.active = newName;
            renderFolders();
        ...
        async function deleteSubFolder(parentName, subName) {
            ...
            StorageService.gmSet(STORAGE_KEYS.BARCODES, barcodes);
            setBarcodesCache(barcodes);
            if (Folders.barcode.activeSub === subName && Folders.barcode.active === parentName) Folders.barcode.activeSub = null;
            renderFolders();
            showFlash('Folder deleted', false, 'success');
        }
        ...
        async function renameSubFolder(parentName, oldName, newName) {
            ...
            StorageService.gmSet(STORAGE_KEYS.BARCODES, barcodes);
            setBarcodesCache(barcodes);
            if (Folders.barcode.activeSub === oldName) Folders.barcode.activeSub = trimmed;
            renderFolders();
        ...
        async function moveFolderTo(folderName, destFolder) {
            ...
            // Single nested folder level only: a folder with its own nested folders can't be demoted.
            if (getSubFolders(folderName).length > 0) {
                showFlash('Move or remove its nested folders first', true, 'error');
                return;
            }
            let folders = await getFolders();
            ...
            StorageService.gmSet(STORAGE_KEYS.FOLDERS, folders);
            setFoldersCache(folders);
            if (Folders.barcode.active === folderName) { Folders.barcode.active = destFolder; Folders.barcode.activeSub = folderName; }
            renderFolders();
        ...
        async function moveSubFolderTo(parentName, subName, destFolder) {
            if (destFolder === parentName) {
                showFlash('Folder is already here', false, 'info');
                return;
            }
            let subFolders = getAllSubFolders();
            let folders = await getFolders();
            ...
            if (!destFolder) {
                ...
                StorageService.gmSet(STORAGE_KEYS.SUBFOLDERS, subFolders);
                if (Folders.barcode.active === parentName && Folders.barcode.activeSub === subName) { Folders.barcode.active = subName; Folders.barcode.activeSub = null; }
                renderFolders();
                showFlash(`Moved "${subName}" to Root`, false, 'success');
                return;
            }

            // Move into another folder as a nested folder.
            if (!folders.some(f => f.name === destFolder)) {
                showFlash('Destination folder not found', true, 'error');
                return;
            }
            ...
            StorageService.gmSet(STORAGE_KEYS.BARCODES, barcodes);
            setBarcodesCache(barcodes);
            if (Folders.barcode.active === parentName && Folders.barcode.activeSub === subName) { Folders.barcode.active = destFolder; }
            renderFolders();
    ...
    // option (folder icon), each nested folder an indented selectable option (nested folder icon).
    // Value format: "folderName::subFolderName" or "folderName::" (no nested folder).
    ...
    function showNewFolderModal(callback) {
        ...
        UI.panel.element.appendChild(modal);
        UI.wireModalIdleTracking(modal);
    ...
    const NoteService = (() => {
        ...
        const NOTE_SUBFOLDERS_KEY = STORAGE_KEYS.NOTE_SUBFOLDERS;
        ...
        function sanitizeNote(raw, fallbackFolderId = '') {
            ...
            const subfolderId = String(raw.subfolderId || raw.subfolder || '').trim();
            return {
        ...
        function sanitizeNoteSubFolder(raw) {
            if (!raw || typeof raw !== 'object') return null;
            const name = String(raw.name || '').trim();
        ...
        function sanitizeNoteSubFolders(list) {
            const byKey = new Map();
            (Array.isArray(list) ? list : []).forEach(raw => {
                const sf = sanitizeNoteSubFolder(raw);
                if (!sf) return;
                const key = `${sf.parent.toLowerCase()}::${sf.name.toLowerCase()}`;
                const existing = byKey.get(key);
                if (!existing || Number(sf.updatedAt || 0) >= Number(existing.updatedAt || 0)) {
                    byKey.set(key, sf);
                }
            });
        ...
        async function moveNoteFolderTo(folderName, destFolder) {
            ...
            if (getNoteSubFolders(folderName).length > 0) {
                showFlash('Move or remove its nested folders first', true, 'error');
                return;
            }
            let folders = getNoteFolders();
            if (!folders.some(f => f.name === destFolder)) {
                showFlash('Destination folder not found', true, 'error');
                return;
            }
            // Move it -> it becomes a subfolder of destFolder
            saveNoteSubFolder(destFolder, folderName);
            deleteNoteFolder(folderName);

            // Update all notes
            const notes = getNotes();
            let modified = false;
            notes.forEach(note => {
                if (note.folderId === folderName && !note.subfolderId) {
                    note.folderId = destFolder;
                    note.subfolderId = folderName;
                    modified = true;
                }
            });
            if (modified) saveNotes(notes);

            if (Folders.note.active === folderName) {
                Folders.note.active = destFolder;
                Folders.note.activeSub = folderName;
            }
            showFlash(`Moved folder "${folderName}" into "${destFolder}"`, false, 'success');
            renderNotes();
        }

        async function moveNoteSubFolderTo(parentName, subName, destFolder) {
            if (!destFolder) {
                // Moving to root
                createNoteFolder(subName);
                deleteNoteSubFolder(parentName, subName);
                const notes = getNotes();
                let modified = false;
                notes.forEach(note => {
                    if (note.folderId === parentName && note.subfolderId === subName) {
                        note.folderId = subName;
                        note.subfolderId = '';
                        modified = true;
                    }
                });
                if (modified) saveNotes(notes);
                if (Folders.note.active === parentName && Folders.note.activeSub === subName) {
                    Folders.note.active = subName;
                    Folders.note.activeSub = null;
                }
                showFlash(`Moved "${subName}" to Root`, false, 'success');
            } else {
                if (destFolder === parentName) {
                    showFlash('Already in that folder', false, 'info');
                    return;
                }
                saveNoteSubFolder(destFolder, subName);
                deleteNoteSubFolder(parentName, subName);
                const notes = getNotes();
                let modified = false;
                notes.forEach(note => {
        ...
        function getAllNoteSubFolders() {
            const explicit = sanitizeNoteSubFolders(StorageService.gmGet(NOTE_SUBFOLDERS_KEY, []));
            const sfKeys = new Set(explicit.map(sf => `${sf.parent.toLowerCase()}::${sf.name.toLowerCase()}`));
            const inferred = [];
            getNotes().forEach(note => {
                const p = String(note.folderId || '').trim();
                const s = String(note.subfolderId || '').trim();
                if (!p || !s) return;
                const k = `${p.toLowerCase()}::${s.toLowerCase()}`;
                if (!sfKeys.has(k)) {
                    sfKeys.add(k);
                    inferred.push({ name: s, parent: p, createdAt: note.createdAt || Date.now(), updatedAt: note.updatedAt || Date.now() });
                }
            });
            return sanitizeNoteSubFolders([...explicit, ...inferred]);
        }

        function getNoteSubFolders(parentName) {
            return getAllNoteSubFolders().filter(sf => sf.parent === parentName);
        }

        function saveNoteSubFolders(sfs) {
            StorageService.gmSet(NOTE_SUBFOLDERS_KEY, sanitizeNoteSubFolders(sfs));
        }

        function saveNoteSubFolder(parentName, subName) {
            const trimmed = String(subName || '').trim();
            if (!trimmed) { showFlash('Name cannot be empty', true, 'error'); return; }
            const all = getAllNoteSubFolders();
            if (all.some(sf => sf.parent === parentName && sf.name.toLowerCase() === trimmed.toLowerCase())) {
                showFlash('Folder already exists', true, 'error');
                return;
            }
            const now = Date.now();
            all.push({ name: trimmed, parent: parentName, createdAt: now, updatedAt: now });
            saveNoteSubFolders(all);
            showFlash(`Folder created: ${trimmed}`, false, 'success');
        }

        function deleteNoteSubFolder(parentName, subName) {
            let all = getAllNoteSubFolders();
            const initial = all.length;
            all = all.filter(sf => !(sf.parent === parentName && sf.name === subName));
            if (all.length !== initial) {
                saveNoteSubFolders(all);

                // Also update notes in this subfolder to just be in the parent
                const notes = getNotes();
                let modified = false;
                notes.forEach(note => {
                    if (note.folderId === parentName && note.subfolderId === subName) {
                ...
                showFlash(`Folder deleted: ${subName}`, false, 'success');
            }
        }
    ...
    // nested folder structure. This mirrors the Barcode folder behavior
    ...
    /* Folders.bookmark.active moved to Folders */
    /* Folders.bookmark.activeSub moved to Folders */
    ...
    function ensureBookmarkDefaults() {
        ...
        if (!defaultHasBookmarks && defaultSubs.length <= 1 && defaultSubs.every(sf => sf.name === BOOKMARK_LEGACY_AUTO_SUBFOLDER)) {
            ...
            if (Folders.bookmark.active === BOOKMARK_LEGACY_AUTO_FOLDER) {
                Folders.bookmark.active = null;
                Folders.bookmark.activeSub = null;
            }
        }
    ...
    function renameBookmarkFolder(oldName, newName) {
        ...
        if (Folders.bookmark.active === oldName) Folders.bookmark.active = trimmed;
        renderBookmarks();
    ...
    function renameBookmarkSubFolder(parentName, oldName, newName) {
        ...
        if (Folders.bookmark.active === parentName && Folders.bookmark.activeSub === oldName) Folders.bookmark.activeSub = trimmed;
        renderBookmarks();
        showFlash('Folder renamed', false, 'success');
    }
    ...
    function deleteBookmarkFolder(folderName) {
        ...
        if (Folders.bookmark.active === folderName) { Folders.bookmark.active = null; Folders.bookmark.activeSub = null; }
        renderBookmarks();
    ...
    function deleteBookmarkSubFolder(parentName, subName) {
        ...
        if (Folders.bookmark.active === parentName && Folders.bookmark.activeSub === subName) Folders.bookmark.activeSub = null;
        renderBookmarks();
        showFlash('Folder deleted', false, 'success');
    }
    ...
    function addOrUpdateBookmark(bookmark) {
        ...
        if (subfolder && !getBookmarkSubFolders(folder).some(sf => sf.name === subfolder)) {
            showFlash('Folder not found', true, 'error');
            return false;
        }
    ...
    function moveBookmarkFolderTo(folderName, destFolder) {
        ...
        if (ownSubs.length > 0 || ownBookmarks.length > 0) {
            showFlash('Move or remove its nested folders/bookmarks first', true, 'error');
            return;
        }
        ...
        if (Folders.bookmark.active === folderName) { Folders.bookmark.active = destFolder; Folders.bookmark.activeSub = folderName; }
        renderBookmarks();
        showFlash(`Moved "${folderName}" into "${destFolder}"`, false, 'success');
    }
    ...
    function moveBookmarkSubFolderTo(parentName, subName, destFolder) {
        ...
        if (!destFolder) {
            ...
            if (subBookmarks.length > 0) {
                showFlash('Move bookmarks out before moving folder to Root', true, 'error');
                return;
            }
            if (!folders.some(f => f.name.toLowerCase() === subName.toLowerCase())) {
                folders.push({ name: subName, pinned: false });
            ...
            saveBookmarkSubFolders(subs);
            if (Folders.bookmark.active === parentName && Folders.bookmark.activeSub === subName) { Folders.bookmark.active = subName; Folders.bookmark.activeSub = null; }
            renderBookmarks();
            showFlash(`Moved "${subName}" to Root`, false, 'success');
            return;
        }
        ...
        if (Folders.bookmark.active === parentName && Folders.bookmark.activeSub === subName) Folders.bookmark.active = destFolder;
        renderBookmarks();
        showFlash(`Moved "${subName}" into "${destFolder}"`, false, 'success');
    }
    ...
    function getDefaultTodoProject() {
        ...
        const stored = gmGet('app:todo:default_project', '');
        if (stored && projects.includes(stored)) {
            return stored;
        }
        return projects[0] || 'Personal';
    }

    function setDefaultTodoProject(name) {
        gmSet('app:todo:default_project', name);
    }
    ...
    function normalizeWellnessSettings(raw) {
        ...
        return {
            ...
            snoozeMinutes: clampWellnessMinutes(data.snoozeMinutes, 5, 1, 60),
    ...
    function showInAppAlarmPopup(title, body, onSnooze, onDismiss) {
        ...
        Object.assign(titleEl.style, { fontWeight: 'bold', fontSize: '14px', color: '#1a73e8' });

        const bodyEl = document.createElement('div');
        bodyEl.textContent = body;
        Object.assign(bodyEl.style, { fontSize: '13px', color: '#333', lineHeight: '1.4' });

        const buttonsRow = document.createElement('div');
        Object.assign(buttonsRow.style, { display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '4px' });

        const dismissBtn = document.createElement('button');
        dismissBtn.textContent = '❌ Dismiss';
        Object.assign(dismissBtn.style, {
            padding: '6px 12px', background: '#f1f3f4', border: 'none', borderRadius: '4px',
            cursor: 'pointer', fontSize: '12px', color: '#333', fontWeight: '500'
        });
        ...
        if (typeof onSnooze === 'function') {
            ...
            snoozeBtn.textContent = `💤 Snooze (${settings.snoozeMinutes}m)`;
            Object.assign(snoozeBtn.style, {
                padding: '6px 12px', background: '#1a73e8', border: 'none', borderRadius: '4px',
                cursor: 'pointer', fontSize: '12px', color: '#fff', fontWeight: '500'
            });
            snoozeBtn.addEventListener('click', () => {
                popup.remove();
                onSnooze();
            });
            buttonsRow.insertBefore(snoozeBtn, dismissBtn);
        }

        popup.append(titleEl, bodyEl, buttonsRow);
        document.body.appendChild(popup);
    }

    function sendAppNotification({ title, body, tag, onClick, fallbackType = 'info', onSnooze, onDismiss }) {
        let sent = false;

        playReminderSound();
        showInAppAlarmPopup(title, body, onSnooze, onDismiss);
        ...
        if (!sent && !sentToWindowsHistory && !onSnooze) {
        ...
        return sent || sentToWindowsHistory || true;
    }
    ...
    function sendWellnessNotification(kind) {
        if (kind === 'water') {
            return sendAppNotification({
                ...
                onSnooze: () => {
                    const settings = getWellnessSettings();
                    settings.waterNextAt = Date.now() + (settings.snoozeMinutes * 60 * 1000);
                    saveWellnessSettings(settings);
                    showFlash(`Water snoozed for ${settings.snoozeMinutes}m`, false, 'info');
                    if (typeof initReminderChecker === 'function') initReminderChecker();
                }
            });
        }
        ...
        return sendAppNotification({
            ...
            onClick: () => window.focus(),
            onSnooze: () => {
                const s = getWellnessSettings();
                s.stretchNextAt = Date.now() + (s.snoozeMinutes * 60 * 1000);
                saveWellnessSettings(s);
                showFlash(`Stretch snoozed for ${s.snoozeMinutes}m`, false, 'info');
                if (typeof initReminderChecker === 'function') initReminderChecker();
            }
        });
    }
    ...
    function runReminderCheck() {
        ...
        tasks.forEach(task => {
            if (!task.completed && !task.archived && task.reminderTime) {
                const rTime = new Date(task.reminderTime).getTime();
                if (!task.reminderSent && now >= rTime) {
                    sendTaskNotification(task);
                    task.reminderSent = true;
                    task.snoozedUntil = null;
                    changed = true;
                }
                if (task.reminderSent && task.snoozedUntil && now >= task.snoozedUntil) {
                    sendTaskNotification(task);
                    task.snoozedUntil = null;
                    changed = true;
                }
    ...
    function sendTaskNotification(task) {
        ...
        return sendAppNotification({
            ...
            onClick: () => {
                window.focus();
                if (typeof UI.togglePanel === 'function' && !UI.panel.isVisible()) {
                    UI.togglePanel();
                }
            ...
            onSnooze: () => {
                const settings = getWellnessSettings();
                snoozeTaskReminder(task.id, settings.snoozeMinutes);
                showFlash(`Task snoozed for ${settings.snoozeMinutes}m`, false, 'info');
                if (typeof initReminderChecker === 'function') initReminderChecker();
    ...
    function showTimerSettingsModal() {
        closeSettingsDropdown();
        const existing = document.getElementById('bm-timer-settings-modal');
        if (existing) existing.remove();

        const modal = document.createElement('div');
        modal.id = 'bm-timer-settings-modal';
        modal.className = 'bm-modal';
        modal.dataset.noAutoclose = '1';
        Object.assign(modal.style, {
            padding: '12px',
        ...
        const header = document.createElement('div');
        header.className = 'bm-header';
        header.textContent = '⏳ Timer & Snooze Settings';
        header.style.fontSize = '14px';
        header.style.marginBottom = '8px';

        const description = document.createElement('div');
        description.className = 'bm-text';
        description.textContent = 'Configure default timeout durations for auto-closing panels and alarm snoozing.';
        description.style.fontSize = '11px';
        description.style.color = '#666';
        description.style.marginBottom = '8px';
        description.style.lineHeight = '1.5';
        ...
        const settings = getWellnessSettings();
        const autoCloseRow = createNumberRow('⏱️ Auto Close', 'Seconds of inactivity before closing panels.', Math.round(getPanelAutoCloseMs() / 1000), 5, 300);
        const snoozeRow = createNumberRow('💤 Snooze duration', 'Minutes to snooze reminders & alarms.', settings.snoozeMinutes, 1, 60);
        ...
        saveBtn.textContent = 'Save';
        saveBtn.addEventListener('click', () => {
            const autoCloseVal = parseInt(autoCloseRow.input.value, 10);
            if (!isNaN(autoCloseVal) && autoCloseVal >= 5) {
                setPanelAutoCloseMs(autoCloseVal * 1000);
            }
            const snoozeVal = parseInt(snoozeRow.input.value, 10);
            if (!isNaN(snoozeVal) && snoozeVal >= 1) {
                settings.snoozeMinutes = clampWellnessMinutes(snoozeVal, settings.snoozeMinutes, 1, 60);
                saveWellnessSettings(settings);
            }
            showFlash('Timer configurations saved', false, 'success');
            modal.remove();
        });
        ...
        buttonsRow.append(saveBtn, cancelBtn);
        modal.append(header, description, autoCloseRow.wrapper, snoozeRow.wrapper, buttonsRow);
        UI.panel.element.appendChild(modal);
        UI.wireModalIdleTracking(modal);
    }
    ...
    async function mergeImportData(incomingFolders, incomingBarcodes, incomingSubFolders = [], options = {}) {
        ...
        Folders.barcode.active = null;
        Folders.barcode.activeSub = null;
        renderFolders();
        if (!options.silent) {
    ...
    function showBookmarkImportFilePicker() {
        ...
        input.addEventListener('change', async () => {
            ...
            try {
                ...
                showFlash(`Bookmark import complete: +${result.addedBookmarkFolders} folders, +${result.addedBookmarkSubFolders} nested folders, +${result.addedBookmarks} bookmarks`, false, 'success');
            } catch (err) {
    ...
    async function buildFullBackupData() {
        ...
        if (!metadata.workspaceId) {
            metadata.workspaceId = generateUniqueId();
            gmSet(STORAGE_KEYS.WORKSPACE_METADATA, metadata);
        }
    ...
    function getPrintViaBrowser() {
        return Boolean(gmGet('bm_print_via_browser', false));
    }

    function setPrintViaBrowser(enabled) {
        gmSet('bm_print_via_browser', Boolean(enabled));
    }

    function getPrintCompact() {
        return Boolean(gmGet('bm_print_compact', false));
    }

    function setPrintCompact(enabled) {
        gmSet('bm_print_compact', Boolean(enabled));
    }
    ...
    function makeBarcodeDigits(value, length) {
        const digits = String(value || '').replace(/\D/g, '').slice(0, length);
        return digits.padEnd(length, '0');
    }

    function getNormalizedBarcodeData(rawValue, rawFormat) {
        const value = String(rawValue || '').trim();
        const format = String(rawFormat || '').toUpperCase();
        if (!value) return { jsbFormat: format, jsbValue: '' };
        let jsbFormat = format;
        let jsbValue = value;
        if (format === 'EAN13') {
            const core = makeBarcodeDigits(value, 12);
            const check = calcEan13CheckDigit(core);
            jsbValue = check != null ? (core + String(check)) : value;
        } else if (format === 'EAN8') {
            const core = makeBarcodeDigits(value, 7);
            const check = calcEan8CheckDigit(core);
            jsbValue = check != null ? (core + String(check)) : value;
        } else if (format === 'UPC') {
            const core = makeBarcodeDigits(value, 11);
            const check = calcUpcCheckDigit(core);
            jsbValue = check != null ? (core + String(check)) : value;
        } else if (format === 'ISBN') {
            const cleaned = value.replace(/[^0-9X]/gi, '').toUpperCase();
            if (cleaned.length > 0 && cleaned.length < 10) {
                const core = makeBarcodeDigits(cleaned, 9);
                const base = `978${core}`;
                const check = calcEan13CheckDigit(base);
                jsbValue = check != null ? (base + String(check)) : base;
                jsbFormat = 'EAN13';
                ...
                jsbValue = check != null ? (cleaned + String(check)) : cleaned;
                jsbFormat = 'EAN13';
            } else {
                jsbValue = value;
                jsbFormat = 'EAN13';
    ...
    let browserPrintQueue = [];
    let browserPrintTimer = null;

    function queueBrowserPrint(params) {
        browserPrintQueue.push(params);
        if (browserPrintTimer) clearTimeout(browserPrintTimer);
        browserPrintTimer = setTimeout(() => {
            const items = [...browserPrintQueue];
            browserPrintQueue = [];
            browserPrintTimer = null;
            printViaBrowser(items);
        }, 100);
    }

    function printViaBrowser(items) {
        if (!items || items.length === 0) return;
        const compact = getPrintCompact();
        ...
        const container = doc.getElementById('labels-container');
        const renderTasks = [];

        items.forEach((item, itemIdx) => {
            const type = item.type || 'barcode';
            ...
            const { jsbFormat, jsbValue } = getNormalizedBarcodeData(data, format);

            for (let i = 0; i < quantity; i++) {
                const page = doc.createElement('div');
                page.className = 'label-page';

                const targetId = `label-gfx-${itemIdx}-${i}`;

                if (type === 'qrcode') {
                    const qrCanvas = doc.createElement('canvas');
                    qrCanvas.id = targetId;
                    page.appendChild(qrCanvas);
                    renderTasks.push(() => {
                        const canvasEl = doc.getElementById(targetId);
                        if (canvasEl && typeof QRCode !== 'undefined') {
                            QRCode.toCanvas(canvasEl, jsbValue, { width: 150, margin: 1 }, (err) => {
                                if (err) console.error(err);
                            });
                        } else if (canvasEl) {
                            const txtNode = doc.createElement('div');
                            txtNode.textContent = jsbValue;
                            canvasEl.replaceWith(txtNode);
                        }
                    });
                } else if (type === 'text') {
                    const txtNode = doc.createElement('div');
                    txtNode.style.whiteSpace = 'pre-wrap';
                    txtNode.style.fontSize = '14px';
                    txtNode.style.textAlign = 'left';
                    txtNode.textContent = desc || text || data;
                    page.appendChild(txtNode);
                } else {
                    const svg = doc.createElementNS('http://www.w3.org/2000/svg', 'svg');
                    svg.setAttribute('id', targetId);
                    page.appendChild(svg);
                    renderTasks.push(() => {
                        const svgEl = doc.getElementById(targetId);
                        if (svgEl && typeof JsBarcode !== 'undefined') {
                            try {
                                JsBarcode(svgEl, jsbValue, {
                                    format: jsbFormat.toUpperCase() === 'QR' ? 'CODE128' : jsbFormat,
                                    width: 2,
                                    height: 60,
                                    displayValue: true
                                });
                                ...
                                txt.textContent = jsbValue;
                                svgEl.replaceWith(txt);
                            }
                        } else if (svgEl) {
                            const txt = doc.createElement('div');
                            txt.textContent = jsbValue;
                ...
                if (type !== 'text') {
                    ...
                    // Barcode Name (Title)
                    const barcodeName = item.name || (type === 'qrcode' ? (text !== data ? text : '') : '');
                    if (barcodeName) {
                        const nameEl = doc.createElement('div');
                        nameEl.className = 'label-text';
                        nameEl.textContent = barcodeName;
                        infoDiv.appendChild(nameEl);
                    }

                    // Barcode Value (only for QR, since linear barcodes show it on the SVG)
                    if (type === 'qrcode') {
                        const valEl = doc.createElement('div');
                        valEl.className = 'label-desc';
                        valEl.textContent = jsbValue;
        ...
        // Run all deferred rendering tasks
        renderTasks.forEach(task => task());

        const hasNonSilent = items.some(x => !x.silent);
        const customSuccessMsg = items.find(x => x.successMessage)?.successMessage;

        setTimeout(() => {
            iframe.contentWindow.focus();
            iframe.contentWindow.print();
            if (hasNonSilent) {
                showFlash(customSuccessMsg || 'Opened browser print dialog', false, 'success');
            }
            setTimeout(() => {
                iframe.remove();
            }, 1000);
    ...
    function sendPrintRequest(params) {
        ...
        if (getPrintViaBrowser()) {
            if (PRINT_LOG?.enabled) {
                PRINT_LOG.add({
                    ...
                    success: true,
                    httpStatus: 200,
                    body: 'browser print queued',
                    error: ''
                });
            }
            queueBrowserPrint(params);
    ...
    function getTargetElement() {
        ...
        if (active && active !== document.body && active !== document.documentElement && !UI.panel.element.contains(active)) {
            return active;
        }
        return document;
    }
    ...
    function showTextPrintModal() {
        ...
        const folderSelect = createFolderDestinationSelect(Folders.barcode.active || 'Default', Folders.barcode.activeSub || '', {
            marginBottom: '0',
            fontSize: '12px',
            padding: '4px 6px',
            height: '28px',
        ...
        saveBtn.addEventListener('click', () => {
            ...
            idbAddBarcode({ name, value: clamped.text, format: 'TEXT', folder, subfolder, pinned: false }).then(() => {
                Folders.barcode.active = folder || null;
                Folders.barcode.activeSub = subfolder || null;
                renderFolders();
    ...
    // UI.panel.element/form/footer variables created later in this same IIFE.
    // ============================================================

    function getPanelAutoCloseMs() {
        return gmGet('app:ui:panel_auto_close_ms', 15000);
    }
    function setPanelAutoCloseMs(ms) {
    ...
    UI.isUserEditing = function () {
        const ae = document.activeElement;
        if (!ae) return false;
        const tag = ae.tagName.toLowerCase();
        if (tag === 'textarea') return true;
        if (tag === 'input') {
            const type = ae.type ? ae.type.toLowerCase() : 'text';
    ...
    UI.panelAutoCloseTimer = null;
    UI.modalAutoCloseTimer = null;
    UI.panelHovering = false;
    UI.modalHovering = false;
    UI.isAnyBmModalOpen = function () {
        return !!document.querySelector('.bm-modal') || !!document.getElementById('bm-barcode-zoom-modal');
    }
    UI.closeAllBmModals = function () {
    ...
    UI.closePanelAndModals = function () {
        UI.closeAllBmModals();
        closeSettingsDropdown();
        closeSearchUI();
        closeDropdown();
        ...
        UI.clearPanelAutoClose();
        UI.clearModalAutoClose();
    }
    UI.isPanelListViewActive = function () {
        if (!UI.panel.isVisible()) return false;
        if (UI.isAnyBmModalOpen()) return false;
        if (!UI.formWrapper.element || !folderDisplay) return true;
        const formHidden = UI.formWrapper.element.style.display === 'none' || window.getComputedStyle(UI.formWrapper.element).display === 'none';
    ...
    UI.schedulePanelAutoClose = function () {
        if (typeof noteFormWrapper !== 'undefined' && noteFormWrapper && noteFormWrapper.dataset.maximized === 'true') {
            UI.clearPanelAutoClose();
            return;
        }
        if (!UI.isPanelListViewActive()) return;
        if (UI.isUserEditing()) {
            UI.clearPanelAutoClose();
            return;
        }
        if (UI.panelAutoCloseTimer) {
            clearTimeout(UI.panelAutoCloseTimer);
        }
        UI.panelAutoCloseTimer = setTimeout(() => {
            if (!UI.isPanelListViewActive() || UI.isUserEditing()) {
                UI.panelAutoCloseTimer = null;
                return;
            }
            closeSettingsDropdown();
    ...
    UI.clearPanelAutoClose = function () {
        if (UI.panelAutoCloseTimer) {
            clearTimeout(UI.panelAutoCloseTimer);
            UI.panelAutoCloseTimer = null;
        }
    ...
    UI.scheduleModalAutoClose = function () {
        if (!UI.isAnyBmModalOpen()) {
            UI.clearModalAutoClose();
            return;
        }
        ...
        UI.clearModalAutoClose();
        UI.modalAutoCloseTimer = setTimeout(() => {
            if (!UI.isAnyBmModalOpen()) return;
            if (document.querySelector('.bm-modal[data-no-autoclose="1"]')) return;
            if (UI.isUserEditing()) {
                UI.clearModalAutoClose();
                return;
            }
            UI.closePanelAndModals();
        }, getModalAutoCloseMs());
    }

    UI.clearModalAutoClose = function () {
        if (UI.modalAutoCloseTimer) {
            clearTimeout(UI.modalAutoCloseTimer);
            UI.modalAutoCloseTimer = null;
        }
    }

    UI.resetModalAutoCloseOnActivity = function () {
        UI.clearModalAutoClose();
        UI.clearPanelAutoClose();
        if (!document.querySelector('.bm-modal[data-no-autoclose="1"]')) {
            UI.scheduleModalAutoClose();
        }
    }

    UI.wireModalIdleTracking = function (modal) {
        if (!modal) return;
        const handler = () => {
            UI.resetModalAutoCloseOnActivity();
        };
    ...
    // order stable unless the whole UI.panel.element lifecycle is tested.
    // ============================================================
    ...
    // Panel
    UI.panel.element = document.createElement('div');
    UI.panel.element.className = 'bm-panel';
    Object.assign(UI.panel.element.style, {
        top: '100px',
        right: '20px',
        width: '280px',
        display: 'none',
    ...
    UI.panel.element.style.position = 'fixed';
    const PANEL_MIN_WIDTH = 460;
    UI.panel.element.style.minWidth = PANEL_MIN_WIDTH + 'px';
    UI.panel.element.style.maxHeight = '80vh';
    UI.panel.element.style.height = UI.panel.element.style.height || '420px';
    UI.panel.element.style.overflow = 'hidden';
    UI.panel.show(); // ensure flex

    const PANEL_OFFSET = 8;
    function updatePanelPosition() {
        const rect = floatingButton.getBoundingClientRect();
        if (rect && Number.isFinite(rect.bottom)) {
            UI.panel.element.style.top = `${Math.round(rect.bottom + PANEL_OFFSET)}px`;
            return;
        }
        const floatingTop = parseInt(floatingContainer.style.top, 10);
        if (!Number.isNaN(floatingTop)) {
            UI.panel.element.style.top = `${floatingTop + 30}px`;
        }
    }
    ...
    UI.panel.element.appendChild(resizeHandle);

    const resetPanelAutoCloseOnActivity = () => {
        if (!UI.isPanelListViewActive()) {
            UI.clearPanelAutoClose();
            return;
        }
        UI.schedulePanelAutoClose();
    ...
    ['mousedown', 'keydown', 'input', 'focusin', 'wheel', 'touchstart'].forEach((evt) => {
        UI.panel.element.addEventListener(evt, resetPanelAutoCloseOnActivity);
    });
    UI.panel.element.addEventListener('mouseenter', () => {
        UI.clearPanelAutoClose();
    });
    UI.panel.element.addEventListener('mouseleave', () => {
        if (UI.panel.isVisible() && !UI.isAnyBmModalOpen()) {
            UI.schedulePanelAutoClose();
        }
    });

    UI.modalObserver = new MutationObserver((mutations) => {
    ...
    UI.modalObserver.observe(document.body, { childList: true, subtree: true });

    let isResizing = false;
    let startX, startY, startWidth, startHeight;

    // Restore UI.panel.element size from storage if exists
    const savedPanelSize = gmGet(STORAGE_KEYS.PANEL_SIZE, null);
    if (savedPanelSize && savedPanelSize.width && savedPanelSize.height) {
        UI.panel.element.style.width = savedPanelSize.width + 'px';
        UI.panel.element.style.height = savedPanelSize.height + 'px';
    }
    ...
    resizeHandle.addEventListener('mousedown', function (e) {
        ...
        startX = e.clientX;
        startY = e.clientY;
        startWidth = parseInt(document.defaultView.getComputedStyle(UI.panel.element).width, 10);
        startHeight = parseInt(document.defaultView.getComputedStyle(UI.panel.element).height, 10);
    ...
    document.addEventListener('mousemove', function (e) {
        ...
        UI.panel.element.style.width = newWidth + 'px';
        UI.panel.element.style.height = newHeight + 'px';
        // Save UI.panel.element size to storage on resize
    ...
    function showPrintServerModal() {
        ...
        const browserPrintRow = document.createElement('div');
        Object.assign(browserPrintRow.style, {
            display: 'flex',
            alignItems: 'center',
            gap: '6px',
            marginTop: '8px',
        ...
        checkbox.id = 'bm-use-browser-print-checkbox';
        checkbox.checked = getPrintViaBrowser();
        checkbox.style.cursor = 'pointer';

        const checkboxLabel = document.createElement('label');
        checkboxLabel.htmlFor = 'bm-use-browser-print-checkbox';
        checkboxLabel.textContent = 'Print via browser (instead of Printmoon)';
        checkboxLabel.style.fontSize = '11px';
        checkboxLabel.style.cursor = 'pointer';
        checkboxLabel.style.color = '#333';
        checkboxLabel.style.userSelect = 'none';

        browserPrintRow.append(checkbox, checkboxLabel);

        const compactPrintRow = document.createElement('div');
        Object.assign(compactPrintRow.style, {
            display: 'flex',
            alignItems: 'center',
            gap: '6px',
            marginTop: '4px',
        ...
        compactCheckbox.id = 'bm-use-compact-print-checkbox';
        compactCheckbox.checked = getPrintCompact();
        compactCheckbox.style.cursor = 'pointer';

        const compactLabel = document.createElement('label');
        compactLabel.htmlFor = 'bm-use-compact-print-checkbox';
        compactLabel.textContent = 'Print all in one page (no page breaks)';
        compactLabel.style.fontSize = '11px';
        compactLabel.style.cursor = 'pointer';
        compactLabel.style.color = '#333';
        compactLabel.style.userSelect = 'none';

        compactPrintRow.append(compactCheckbox, compactLabel);

        const updateCompactState = () => {
            compactCheckbox.disabled = !checkbox.checked;
            compactLabel.style.opacity = checkbox.checked ? '1' : '0.5';
            compactLabel.style.cursor = checkbox.checked ? 'pointer' : 'not-allowed';
            compactCheckbox.style.cursor = checkbox.checked ? 'pointer' : 'not-allowed';
        };
        checkbox.addEventListener('change', updateCompactState);
        updateCompactState();
        ...
        saveBtn.addEventListener('click', () => {
            ...
            setPrintViaBrowser(checkbox.checked);
            setPrintCompact(compactCheckbox.checked);
            showFlash('Print settings saved', false, 'success');
            modal.remove();
        });
        ...
        clearBtn.addEventListener('click', () => {
            ...
            setPrintViaBrowser(false);
            setPrintCompact(false);
            showFlash('Print settings cleared', false, 'success');
            modal.remove();
        });
        ...
        modal.append(header, hint, overrideLabel, input, browserPrintRow, compactPrintRow, buttonsRow);
        UI.panel.element.appendChild(modal);
        UI.wireModalIdleTracking(modal);
    ...
    function applyTabVisibility() {
        const hiddenTabs = gmGet('app:ui:hidden_tabs', []);
        tabsMap.forEach((tabInfo, name) => {
            if (hiddenTabs.includes(name)) {
                tabInfo.button.style.setProperty('display', 'none', 'important');
            } else {
                tabInfo.button.style.removeProperty('display');
            }
        });

        // If current tab is hidden, switch to the first visible tab
        if (hiddenTabs.includes(currentTabName)) {
            const firstVisible = Array.from(tabsMap.keys()).find(name => !hiddenTabs.includes(name));
            if (firstVisible) {
                UI.switchTab(firstVisible);
            }
        }
    }

    function updateTabVisibility(tabName, hide) {
        const hiddenTabs = gmGet('app:ui:hidden_tabs', []);
        const idx = hiddenTabs.indexOf(tabName);
        if (hide && idx === -1) {
            hiddenTabs.push(tabName);
        } else if (!hide && idx !== -1) {
            hiddenTabs.splice(idx, 1);
        }
        gmSet('app:ui:hidden_tabs', hiddenTabs);
        applyTabVisibility();
    }

    function createSettingsPanel() {
        const container = document.createElement('div');
        container.className = 'pa-settings-container bm-menu bm-settings-dropdown';
        ...
        // ==========================================
        // ☁ SECTION 1: Cloud & Workspace Status
        // ==========================================
        const wsSection = document.createElement('div');
        wsSection.className = 'pa-section';

        const wsTitle = document.createElement('div');
        wsTitle.className = 'pa-section-title';
        wsTitle.textContent = 'Cloud & Workspace Status';
        wsSection.appendChild(wsTitle);

        // Alert/Warning banners
        const browserBanner = document.createElement('div');
        browserBanner.className = 'pa-alert-banner';
        wsSection.appendChild(browserBanner);

        const guidanceBanner = document.createElement('div');
        guidanceBanner.className = 'pa-alert-banner';
        guidanceBanner.textContent = getWorkspaceOneDriveGuidance();
        wsSection.appendChild(guidanceBanner);

        // Status grid items
        const statusSpans = {};
        const addStatusRow = (label) => {
            const row = document.createElement('div');
            row.className = 'pa-status-grid';

            const labelSpan = document.createElement('span');
            labelSpan.className = 'pa-status-label';
            labelSpan.textContent = label;

            const valueSpan = document.createElement('span');
            valueSpan.className = 'pa-status-value';
            valueSpan.textContent = '—';

            row.append(labelSpan, valueSpan);
            wsSection.appendChild(row);
            statusSpans[label] = valueSpan;
        };

        addStatusRow('Status');
        addStatusRow('Supported Browser');
        addStatusRow('Selected Folder');
        addStatusRow('Permission');
        addStatusRow('Last Save');
        addStatusRow('Auto Backup');

        // Connection Action Buttons
        const wsBtnGroup = document.createElement('div');
        wsBtnGroup.className = 'pa-btn-group';

        const toggleConnectBtn = document.createElement('button');
        toggleConnectBtn.style.gridColumn = 'span 2';
        toggleConnectBtn.title = getWorkspaceOneDriveGuidance();
        ...
        wsBtnGroup.append(toggleConnectBtn, saveBtn, restoreBtn);
        wsSection.appendChild(wsBtnGroup);
        ...
        dataSection.className = 'pa-section';

        const dataTitle = document.createElement('div');
        dataTitle.className = 'pa-section-title';
        dataTitle.textContent = 'Data & Storage';
        dataSection.appendChild(dataTitle);

        const dataBtnGroup = document.createElement('div');
        dataBtnGroup.className = 'pa-btn-group';
        ...
        const originalExportClick = async function (e) {
            ...
            try {
                ...
                const filename = `Workspace_${dateStr}.pa`;
                ...
                exportBtn.textContent = 'Ready (Click Here)';
                ...
                exportBtn.onclick = function (ev) {
                    ...
                    a2.click();
                    setTimeout(() => a2.remove(), 100);

                    showFlash('Workspace exported', false, 'success');
                    closeSettingsDropdown();
        ...
        dataBtnGroup.append(importBtn, exportBtn);
        dataSection.appendChild(dataBtnGroup);

        const resetBtn = document.createElement('button');
        resetBtn.className = 'pa-danger-btn';
        ...
        visibilitySection.className = 'pa-section';

        const visibilityTitle = document.createElement('div');
        visibilityTitle.className = 'pa-section-title';
        visibilityTitle.textContent = 'Interface Tabs';
        ...
        tabList.forEach(tab => {
            ...
            const checkbox = document.createElement('input');
            checkbox.type = 'checkbox';
            checkbox.checked = !hiddenTabs.includes(tab.id);
            ...
            checkbox.addEventListener('change', () => {
                updateTabVisibility(tab.id, !checkbox.checked);
            });
        ...
        // ==========================================
        // 🔧 SECTION 3: Utilities & Extras
        // ==========================================
        const utilsSection = document.createElement('div');
        utilsSection.className = 'pa-section';

        const utilsTitle = document.createElement('div');
        utilsTitle.className = 'pa-section-title';
        utilsTitle.textContent = 'Utilities & Extras';
        utilsSection.appendChild(utilsTitle);

        const utilsBtnGroup = document.createElement('div');
        utilsBtnGroup.className = 'pa-btn-group';
        ...
        const timerSettingsBtn = document.createElement('button');
        timerSettingsBtn.textContent = 'Time Settings';
        timerSettingsBtn.onclick = function () {
        ...
        const printLogBtn = document.createElement('button');
        ...
        if (PRINT_LOG?.enabled && typeof PRINT_LOG.showModal === 'function') {
            ...
            printLogBtn.disabled = true;
            printLogBtn.style.opacity = '0.5';
            printLogBtn.style.cursor = 'not-allowed';
            printLogBtn.title = 'Print Log is not enabled';
        }

        utilsBtnGroup.append(wellnessBtn, timerSettingsBtn, printServerBtn, printLogBtn);
        utilsSection.appendChild(utilsBtnGroup);

        container.appendChild(utilsSection);

        // LIVE STATUS UPDATE LOGIC
        const setBusy = (busy) => {
            [toggleConnectBtn, saveBtn, restoreBtn].forEach(btn => {
                btn.disabled = !!busy;
        ...
        const update = () => {
            ...
            browserBanner.textContent = getWorkspaceBrowserMessage();

            const kind = getWorkspaceBrowserKind();
            if (kind === 'chromium') {
                browserBanner.style.background = 'rgba(16, 185, 129, 0.1)';
                browserBanner.style.borderLeft = '3px solid #10b981';
                browserBanner.style.color = '#065f46';
            } else {
                browserBanner.style.background = 'rgba(245, 158, 11, 0.1)';
                browserBanner.style.borderLeft = '3px solid #f59e0b';
                browserBanner.style.color = '#9a3412';
            }

            // Status Row with dynamic glowing status dot
            statusSpans.Status.innerHTML = '';
            const dot = document.createElement('span');
            dot.className = 'pa-status-dot';
            if (state.state === WorkspaceState.CONNECTED || state.state === WorkspaceState.READY) {
                dot.classList.add('ready');
            } else if (state.state === WorkspaceState.CONNECTING || state.state === WorkspaceState.PERMISSION_REQUIRED) {
                dot.classList.add('warn');
            } else {
                dot.classList.add('disconnected');
            }
            const statusText = document.createTextNode(' ' + formatWorkspaceStateValue(state.state));
            statusSpans.Status.append(dot, statusText);

            statusSpans['Supported Browser'].textContent = state.supported ? 'Yes' : 'No';
            statusSpans['Selected Folder'].textContent = state.selectedFolder || 'None';
            statusSpans['Selected Folder'].title = state.selectedFolder || '';
            statusSpans.Permission.textContent = formatWorkspaceStateValue(state.permission || 'unknown');
            statusSpans['Last Save'].textContent = formatWorkspaceDate(state.lastSaveAt);
            statusSpans['Last Save'].title = state.lastSavePath || '';

            if (state.snapshotInProgress) {
                statusSpans['Auto Backup'].textContent = 'Saving...';
                statusSpans['Auto Backup'].title = 'Automatic snapshot is being written now.';
            } else if (state.snapshotDueAt) {
                statusSpans['Auto Backup'].textContent = `${state.snapshotDirty ? 'Pending' : 'Next'} ${formatWorkspaceDate(state.snapshotDueAt)}`;
                statusSpans['Auto Backup'].title = `Next automatic snapshot is scheduled for ${state.snapshotDueAt}`;
            } else if (state.lastSnapshotAt) {
                statusSpans['Auto Backup'].textContent = `${formatWorkspaceDate(state.lastSnapshotAt)} (${state.snapshotCount || 0}/${state.maxSnapshots || 10})`;
                statusSpans['Auto Backup'].title = state.lastSnapshotPath || '';
            } else {
                statusSpans['Auto Backup'].textContent = `Every ${Math.round((state.snapshotIntervalMs || 600000) / 60000)} min, max ${state.maxSnapshots || 10}`;
                statusSpans['Auto Backup'].title = 'Automatic snapshots are saved to PA/backups after data changes when Workspace is connected and permission is granted.';
            }

            const hasWorkspace = state.state !== WorkspaceState.DISCONNECTED && !!state.selectedFolder;

            // Toggle connect button text and styling based on connection status
            if (hasWorkspace) {
                toggleConnectBtn.textContent = 'Disconnect';
                toggleConnectBtn.style.setProperty('background', '#fef2f2', 'important');
                toggleConnectBtn.style.setProperty('color', '#b91c1c', 'important');
                toggleConnectBtn.style.setProperty('border-color', '#fca5a5', 'important');
            } else {
                toggleConnectBtn.textContent = 'Connect';
                toggleConnectBtn.style.setProperty('background', '#1f4e79', 'important');
                toggleConnectBtn.style.setProperty('color', '#ffffff', 'important');
                toggleConnectBtn.style.setProperty('border-color', '#1f4e79', 'important');
            }

            toggleConnectBtn.disabled = !state.supported;
        ...
        const unbindWorkspaceListener = WorkspaceService.on('*', () => {
        ...
        // Disconnect listener when panel is removed from DOM
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                mutation.removedNodes.forEach((node) => {
                    if (node === container) {
                        unbindWorkspaceListener();
                        observer.disconnect();
                    }
                });
            });
        ...
        toggleConnectBtn.addEventListener('click', async (event) => {
    ...
    settingsButton.onclick = function (e) {
        ...
        document.querySelectorAll('.bm-settings-dropdown, .pa-settings-container').forEach(el => el.remove());

        settingsDropdown = createSettingsPanel();
    ...
    UI.panel.element.appendChild(searchHost);
    ...
    async function renderBarcodeSearchResults(query) {
        ...
        if (matchedFolders.length > 0) {
            ...
            matchedFolders.forEach(folder => {
                ...
                div.onclick = async (e) => {
                    e.stopPropagation();
                    Folders.barcode.active = folder.name;
                    renderFolders();
        ...
        if (matchedBarcodes.length > 0) {
            ...
            `;
                ...
                div.onclick = async (e) => {
                    e.stopPropagation();
                    Folders.barcode.active = b.folder;
                    closeSearchUI();
                    await renderFolders();
    ...
    // Only add to UI.panel.element once
    if (!UI.panel.element.querySelector('div[style*="display: flex"][style*="flex-direction: row"]')) {
    ...
    // ------------------------------------------------------------
    // UI.formWrapper.element, folderDisplay, Folders.barcode.active, selectedBarcodeIds,
    ...
    UI.panel.element.appendChild(tabBar);
    ...
    UI.panel.element.appendChild(tabContentContainer);
    ...
    UI.registerTab = function (name, label, contentElement) {
        const tabButton = document.createElement('button');
        tabButton.className = 'bm-tab';
        tabButton.textContent = label;
        tabButton.addEventListener('click', () => UI.switchTab(name));
    ...
    window.bmRegisterTab = UI.registerTab;
    window.bmSwitchTab = UI.switchTab;
    ...
    UI.formWrapper.element = document.createElement('div');
    // Make UI.formWrapper.element flexible and scrollable, but hidden by default
    Object.assign(UI.formWrapper.element.style, {
        flex: '0 0 auto',
        overflow: 'visible',
        minHeight: '0',
    ...
    barcodeTabContent.appendChild(UI.formWrapper.element);

    UI.flashMessage.element = document.createElement('div');
    UI.flashMessage.element.className = 'bm-flash';
    window._barcodeFlash = UI.flashMessage.element;
    UI.formWrapper.element.appendChild(UI.flashMessage.element);
    ...
    UI.registerTab('barcode', '📊 Barcode', barcodeTabContent);
    ...
    UI.registerTab('bookmarks', '🔖 Bookmarks', bookmarksTabContent);
    ...
    function showBookmarkFolderForm() {
        ...
        hint.textContent = 'Nested folders are created manually inside the folder.';
    ...
    function showBookmarkForm(bookmarkToEdit = null) {
        ...
        const destSelect = createBookmarkDestinationSelect(
            bookmarkToEdit?.folder || Folders.bookmark.active || '',
            bookmarkToEdit?.subfolder || Folders.bookmark.activeSub || ''
        );
        ...
        saveBtn.addEventListener('click', () => {
            ...
            if (!ok) return;
            Folders.bookmark.active = destination.folder;
            Folders.bookmark.activeSub = destination.subfolder;
            bookmarkFormWrapper.style.display = 'none';
            bookmarkDisplay.style.display = 'flex';
            renderBookmarks();
    ...
    function showBookmarkMoveModal(type, name, parentName = null, bookmark = null) {
        const folders = getBookmarkFolders();
        const subfolders = getAllBookmarkSubFolders();
        const hasSubfolders = type === 'folder' && subfolders.some(sf => sf.parent === name);
        let title = `Move Bookmark ${type} (${name})`;
        if (type === 'bookmark') title = `Move Bookmark`;

        showUniversalMoveModal({
            title,
            itemType: type === 'bookmark' ? 'file' : type,
            itemName: name,
            hasSubfolders,
            folders,
            subfolders,
            onApply: (destFolder, destSub) => {
                if (type === 'folder') moveBookmarkFolderTo(name, destFolder);
                else if (type === 'subfolder') moveBookmarkSubFolderTo(parentName, name, destFolder);
                else if (type === 'bookmark' && bookmark) {
                    if (!destFolder) {
                        showFlash('Select a destination folder', true, 'error');
                        return;
                    }
                    const changed = updateBookmarksByIds([bookmark.id], { folder: destFolder, subfolder: destSub || '' });
                    if (changed) {
                        Folders.bookmark.active = destFolder;
                        Folders.bookmark.activeSub = destSub || '';
                        renderBookmarksList();
    ...
    function showBookmarkBatchMoveModal(ids, onDone) {
        ...
        showUniversalMoveModal({
            title: `Move ${moveIds.length} bookmark(s)`,
            itemType: 'file',
            folders,
            subfolders,
            onApply: (destFolder, destSub) => {
                if (!destFolder) {
                    showFlash('Select a destination folder first', true, 'error');
                    return;
                }
                const changed = updateBookmarksByIds(moveIds, { folder: destFolder, subfolder: destSub || '' });
                selectedBookmarkIds.clear();
                Folders.bookmark.active = destFolder;
                Folders.bookmark.activeSub = destSub || '';
                renderBookmarks();
                if (typeof onDone === 'function') onDone(changed, { folder: destFolder, subfolder: destSub || '' });
    ...
    function createBookmarkFolderIcon(hasContent, blue = false) {
        return UI.createFolderIconSVG(hasContent, blue);
    }
    ...
    function renderBookmarks() {
        ...
        const bookmarkSvg = `<svg width="16" height="16" viewBox="0 0 24 24" style="vertical-align: middle; display: inline-block; margin-right: 4px;"><path fill="#1976d2" d="M6 3h12a1 1 0 0 1 1 1v17l-7-4-7 4V4a1 1 0 0 1 1-1z"></path><path fill="#ffffff" d="M8 6h8v2H8V6zm0 4h8v2H8v-2z" opacity="0.85"></path></svg>`;
        const newBtn = UI.createToolbarButton('New Bookmark', bookmarkSvg, () => showBookmarkForm());
        const newFolderBtn = UI.createToolbarButton('New folder', '📁', () => {
            if (Folders.bookmark.active) {
                showNewFolderModal((name) => { if (name) { saveBookmarkSubFolder(Folders.bookmark.active, name); renderBookmarks(); } });
            } else {
                showBookmarkFolderForm();
            }
        });
        if (Folders.bookmark.activeSub) {
            newFolderBtn.style.display = 'none';
        }
        const importBtn = UI.createToolbarButton('Import', '📥', showBookmarkImportFilePicker);
        importBtn.title = 'Import browser-exported bookmarks HTML or Firefox JSON';
        importBtn.style.marginLeft = 'auto';

        toolbar.append(newBtn, newFolderBtn, importBtn);
        bookmarkDisplay.appendChild(toolbar);

        let list = bookmarks.filter(b => !!b.folder); // Only valid bookmarks

        if (!query && Folders.bookmark.active) {
            list = list.filter(b => b.folder === Folders.bookmark.active && (Folders.bookmark.activeSub ? b.subfolder === Folders.bookmark.activeSub : !b.subfolder));
        } else if (query) {
            list = list.filter(b => b.name.toLowerCase().includes(query) || (b.url && b.url.toLowerCase().includes(query)));
        }

        if (!query && !Folders.bookmark.active) {
            const folders = getBookmarkFolders().map(folder => {
                const subCount = getBookmarkSubFolders(folder.name).length;
                const count = bookmarks.filter(b => b.folder === folder.name).length;
                return { ...folder, hasContent: count > 0 || subCount > 0 };
            });
            ...
            UI.renderFolderGrid(folders, bookmarkDisplay,
                (name) => {
                    Folders.bookmark.active = name;
                    Folders.bookmark.activeSub = null;
                    renderBookmarks();
                },
                (folder, menu, closeMenu) => {
        ...
        if (Folders.bookmark.active && !query) {
            const headerRow = document.createElement('div');
            Object.assign(headerRow.style, { display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: '8px', width: '100%', marginBottom: '10px' });
            const backBtn = document.createElement('button');
            backBtn.className = 'bm-button';
            backBtn.textContent = '⬅Back';
            Object.assign(backBtn.style, { marginBottom: '0', flex: '0 0 auto', visibility: 'visible' });
            backBtn.addEventListener('click', () => {
                if (Folders.bookmark.activeSub) Folders.bookmark.activeSub = null;
                else Folders.bookmark.active = null;
                renderBookmarks();
            });
            const titleDiv = document.createElement('div');
            titleDiv.className = 'bm-header folder-name';
            titleDiv.textContent = Folders.bookmark.activeSub ? `${Folders.bookmark.active} / ${Folders.bookmark.activeSub}` : Folders.bookmark.active;
            Object.assign(titleDiv.style, {
                fontWeight: 'bold', fontSize: '13px', color: '#000', letterSpacing: '1px',
                textAlign: 'center', margin: '0', display: 'block', flex: '1 1 0%', minWidth: '0'
            });
            const placeholder = document.createElement('div');
            Object.assign(placeholder.style, { flex: '0 0 auto', width: '60px', visibility: 'hidden' });

            headerRow.append(backBtn, titleDiv, placeholder);
            bookmarkDisplay.appendChild(headerRow);

            if (!Folders.bookmark.activeSub) {
                const subs = getBookmarkSubFolders(Folders.bookmark.active).map(sf => {
                    const count = bookmarks.filter(b => b.folder === Folders.bookmark.active && b.subfolder === sf.name).length;
                    return { ...sf, hasContent: count > 0 };
                });
                subs.sort((a, b) => {
                ...
                UI.renderSubFolderGrid(subs, bookmarkDisplay,
                    (name) => { Folders.bookmark.activeSub = name; renderBookmarks(); },
                    (sf, menu, closeMenu) => {
                        const renameBtn = document.createElement('button'); renameBtn.className = 'bm-button'; renameBtn.textContent = '✏️ Rename'; renameBtn.onclick = () => { closeMenu(); showRenameModal(sf.name, n => { if (n) renameBookmarkSubFolder(Folders.bookmark.active, sf.name, n); }); };
                        const moveBtn = document.createElement('button'); moveBtn.className = 'bm-button'; moveBtn.textContent = '📂 Move'; moveBtn.onclick = () => { closeMenu(); showBookmarkMoveModal('subfolder', sf.name, Folders.bookmark.active); };
                        const delBtn = document.createElement('button'); delBtn.className = 'bm-button'; delBtn.textContent = '🗑️ Delete'; delBtn.onclick = () => { closeMenu(); bmConfirm(`Delete bookmark folder "${sf.name}"?`, ok => { if (ok) deleteBookmarkSubFolder(Folders.bookmark.active, sf.name); }); };
                        const pinBtn = document.createElement('button'); pinBtn.className = 'bm-button'; pinBtn.textContent = sf.pinned ? '📌 Unpin' : '📌 Pin'; pinBtn.onclick = () => { closeMenu(); updateBookmarkSubFolder(Folders.bookmark.active, sf.name, { pinned: !sf.pinned }); };
                        menu.append(renameBtn, moveBtn, delBtn, pinBtn);
                    }
                );
            }
        ...
        if (selectedVisibleCount > 0 && list.length > 0) {
            ...
            copyBtn.addEventListener('click', () => {
                const selectedUrls = list.filter(bookmark => selectedBookmarkIds.has(String(bookmark.id))).map(bookmark => bookmark.url).filter(url => String(url || '').trim());
                if (!selectedUrls.length) { showFlash('Nothing to copy', true, 'error'); return; }
        ...
        Object.assign(listWrap.style, { display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '0', width: '100%', alignContent: 'flex-start', justifyContent: 'flex-start' });
        list.forEach(bookmark => listWrap.appendChild(createBookmarkItem(bookmark)));
        bookmarkDisplay.appendChild(listWrap);

        if (!list.length && (Folders.bookmark.active || query)) {
            const empty = document.createElement('div');
            empty.className = 'bm-text';
    ...
    function createBookmarkItem(bookmark) {
        ...
        let icon;
        if (bookmark.faviconCachedUrl === 'FAILED') {
            icon = document.createElement('div');
            icon.textContent = '🔗';
            Object.assign(icon.style, {
                fontSize: '18px', width: '24px', height: '24px',
                display: 'flex', alignItems: 'center', justifyContent: 'center'
            });
        } else {
            icon = document.createElement('img');
            icon.src = bookmark.faviconCachedUrl || bookmark.favicon || getBookmarkFaviconUrl(bookmark.url);
            icon.width = 24;
            icon.height = 24;
            icon.style.borderRadius = '4px';

            icon.onload = () => {
                if (bookmark.faviconCachedUrl !== icon.src) {
                    bookmark.faviconCachedUrl = icon.src;
                    updateBookmark(bookmark.id, { faviconCachedUrl: icon.src }, true);
                }
            };
            ...
            icon.onerror = () => {
                ...
                if (fallback && icon.src !== fallback) {
                    ...
                    updateBookmark(bookmark.id, { faviconCachedUrl: 'FAILED' }, true);
                    bookmark.faviconCachedUrl = 'FAILED';
    ...
    UI.registerTab('notes', '📓 Notebook', notesTabContent);

    let activeNoteId = null;
    /* Folders.note.active moved to Folders */
    let selectedNoteIds = new Set();
    let noteSearchQuery = '';
    let noteSortMode = 'updated-desc';
    let showArchivedNotes = false;

    function populateNoteFolderSelect(select, preferredFolder = '', preferredSubfolder = '') {
        const folders = NoteService.getNoteFolders().sort((a, b) => (b.pinned ? 1 : 0) - (a.pinned ? 1 : 0) || a.name.localeCompare(b.name));
        const subfolders = NoteService.getAllNoteSubFolders();
        select.innerHTML = '';
        folders.forEach(folder => {
            const opt = document.createElement('option');
            ...
            const subs = subfolders.filter(sf => sf.parent === folder.name).sort((a, b) => a.name.localeCompare(b.name));
            subs.forEach(sf => {
                const subOpt = document.createElement('option');
                subOpt.value = `${folder.name}::${sf.name}`;
        ...
        const preferred = preferredSubfolder ? `${preferredFolder}::${preferredSubfolder}` : String(preferredFolder || '').trim();
        const exists = folders.some(f => f.name === preferred) || subfolders.some(sf => `${sf.parent}::${sf.name}` === preferred);
        select.value = exists ? preferred : (folders[0]?.name || '__NEW__');
        select.dataset.lastValid = select.value === '__NEW__' ? '' : select.value;
    }

    function createNoteFolderSelect(preferredFolder = '', preferredSubfolder = '') {
        const select = document.createElement('select');
        select.className = 'bm-input';
        select.style.width = '100%';
        select.style.boxSizing = 'border-box';
        populateNoteFolderSelect(select, preferredFolder, preferredSubfolder);
    ...
    async function showNoteEditor(noteToEdit = null) {
        ...
        const folderSelect = createNoteFolderSelect(
            noteToEdit?.folderId || Folders.note.active || '',
            noteToEdit?.subfolderId || Folders.note.activeSub || ''
        );
        folderSelect.style.marginBottom = '0';
        ...
        contentEditor.dataset.placeholder = 'Write your note… (paste a screenshot directly, or use Img to attach an image)';
        ...
        function clearSelectedNoteImage() {
            if (selectedNoteImage) {
                selectedNoteImage.classList.remove('bm-note-img-selected');
        ...
        function selectNoteImage(img) {
            ...
            img.classList.add('bm-note-img-selected');
        ...
        function decorateNoteImage(img) {
            if (!img || img.tagName !== 'IMG' || img.dataset.noteImgBound === '1') return;
            img.dataset.noteImgBound = '1';
            img.classList.add('bm-note-img');
            img.draggable = false;
        ...
        function decorateNoteImages(root) {
            (root || contentEditor).querySelectorAll('img').forEach(decorateNoteImage);
        }

        if (noteToEdit) {
            decorateNoteImages(contentEditor);
        }
        ...
        function insertImageFile(file) {
            ...
            reader.onload = () => {
                ...
                document.execCommand('insertHTML', false,
                    `<img src="${reader.result}" class="bm-note-img" style="max-width:100%;height:auto;border-radius:4px;margin:4px 0;display:block;">`);
                const imgs = contentEditor.querySelectorAll('img');
                const inserted = imgs[imgs.length - 1];
                if (inserted) {
                    decorateNoteImage(inserted);
                    selectNoteImage(inserted);
                }
            };
        ...
        // Paste screenshots/images, or auto-link pasted URLs
        contentEditor.addEventListener('paste', (e) => {
            ...
            const pastedUrl = isPasteableUrl(clipboard?.getData('text/plain'));
            if (pastedUrl) {
                e.preventDefault();
        ...
        contentEditor.addEventListener('keydown', (e) => {
            if ((e.key === 'Delete' || e.key === 'Backspace') && selectedNoteImage) {
                e.preventDefault();
                selectedNoteImage.remove();
                selectedNoteImage = null;
            }
        });
        ...
        ['11', '12', '13', '14', '16', '18', '20', '24'].forEach(size => {
            const opt = document.createElement('option');
            opt.value = size;
        ...
        async function saveFromEditor() {
            const val = String(folderSelect.value || folderSelect.dataset.lastValid || '').trim();
            if (!val || val === '__NEW__' || folderSelect.dataset.noFolders === '1') {
                showFlash('Create/select a notebook folder first', true, 'error');
                return false;
            }
            const parsed = parseTreeSelectValue(val, 'Default');
            const folderId = parsed.folder;
            const subfolderId = parsed.subfolder;
            ...
            if (isMaximized && typeof maximizeBtn !== 'undefined') {
                toggleMaximize(maximizeBtn);
            }

            activeNoteId = saved.id;
            Folders.note.active = saved.folderId || Folders.note.active;
            Folders.note.activeSub = saved.subfolderId || Folders.note.activeSub;
            noteFormWrapper.style.display = 'none';
            noteDisplay.style.display = 'flex';
            await renderNotes();
        ...
        const saveBtn = mkToolBtn(
            '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#2e7d32" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; display: inline-block;"><polyline points="20 6 9 17 4 12"/></svg>',
            noteToEdit ? 'Update' : 'Create',
            saveFromEditor
        );

        const cancelBtn = mkToolBtn(
            '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#d32f2f" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; display: inline-block;"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>',
            'Cancel',
            () => {
                if (isMaximized && typeof maximizeBtn !== 'undefined') {
                    toggleMaximize(maximizeBtn);
        ...
        const printBtn = mkToolBtn(
            '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#0288d1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; display: inline-block;"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>',
            'Print note',
            () => printNote(titleInput.value.trim(), contentEditor.innerHTML)
        );

        function toggleMaximize(btn) {
            isMaximized = !isMaximized;
            noteFormWrapper.dataset.maximized = isMaximized ? 'true' : 'false';
            if (isMaximized) {
                noteFormWrapper.dataset.origStyle = noteFormWrapper.getAttribute('style') || '';
                form.dataset.origStyle = form.getAttribute('style') || '';
                editorWrap.dataset.origStyle = editorWrap.getAttribute('style') || '';
                contentEditor.dataset.origStyle = contentEditor.getAttribute('style') || '';
                ...
                btn.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#2196f3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; display: inline-block;"><path d="M4 14h6v6m10-10h-6V4m-6 16l-7-7m17 7l7-7"/></svg>';
                btn.title = 'Restore';
                UI.clearPanelAutoClose();
            } else {
                noteFormWrapper.setAttribute('style', noteFormWrapper.dataset.origStyle || '');
                form.setAttribute('style', form.dataset.origStyle || '');
                editorWrap.setAttribute('style', editorWrap.dataset.origStyle || '');
                contentEditor.setAttribute('style', contentEditor.dataset.origStyle || '');

                btn.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#2196f3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; display: inline-block;"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"/></svg>';
                btn.title = 'Maximize';
                if (UI.panel.element && UI.panel.isVisible()) {
                    UI.schedulePanelAutoClose();
                }
            }
        }

        const maximizeBtn = mkToolBtn(
            '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#2196f3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; display: inline-block;"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"/></svg>',
            'Maximize',
            (btn) => toggleMaximize(btn)
        );
        ...
        toolbar.append(
            ...
            fontSizeSelect,
            mkSep(),
            mkToolBtn('<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#2196f3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; display: inline-block;"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>', 'Attach image', () => imageInput.click()),
            printBtn,
            maximizeBtn,
            mkSep(),
            saveBtn,
        ...
        meta.textContent = noteToEdit ? `Updated: ${formatNoteDate(noteToEdit.updatedAt)}` : 'Rich text: format text, change font size, paste links or screenshots, attach images.';
        ...
        form.append(header, titleInput, folderSelect, tagsInput, editorWrap, meta);
    ...
    function showNoteBatchMoveModal(ids, onDone) {
        ...
        const folders = NoteService.getNoteFolders();
        const subfolders = NoteService.getNoteSubFolders();

        showUniversalMoveModal({
            title: `Move ${moveIds.length} note(s)`,
            itemType: 'file',
            folders,
            subfolders,
            onApply: (destFolder, destSub) => {
                ...
                moveIds.forEach(id => {
                    ...
                    if (note && (note.folderId !== destFolder || note.subfolderId !== destSub)) {
                        NoteService.updateNote(id, { folderId: destFolder, subfolderId: destSub || '' });
                        changed++;
                    }
                });
                selectedNoteIds.clear();
                Folders.note.active = destFolder;
                Folders.note.activeSub = destSub || '';
                renderNotes();
                if (typeof onDone === 'function') onDone(changed, destFolder);
    ...
    function createNoteCard(note) {
        ...
        icon.innerHTML = `
            <svg width="24" height="24" viewBox="0 0 24 24" style="display: block;">
                <path fill="#1976d2" d="M6 3h10l3 3v15H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2z"/>
        ...
        text.append(titleEl, summary, meta);
        ...
        card.append(batchCheckbox, icon, text, menuIcon);
    ...
    async function renderNotes(options = {}) {
        ...
        const newBtn = UI.createToolbarButton('New Note', '📝', () => showNoteEditor());
        const newFolderBtn = UI.createToolbarButton('New folder', '📁', () => {
            showNewFolderModal((name) => {
                if (!name) return;
                if (Folders.note.active) {
                    NoteService.saveNoteSubFolder(Folders.note.active, name);
                    renderNotes();
                } else {
                    const ok = NoteService.createNoteFolder(name);
                    if (ok) {
                        Folders.note.active = name;
                        renderNotes();
                    }
                }
            });
        });
        if (Folders.note.activeSub) {
            newFolderBtn.style.display = 'none';
        }
        const archiveBtn = UI.createToolbarButton(showArchivedNotes ? 'Active Notes' : 'Archive', '📦', () => {
        ...
        if (!noteSearchQuery && !Folders.note.active && !showArchivedNotes) {
            const folders = allFolders.map(folder => {
                const count = allNotes.filter(n => !n.archived && n.folderId === folder.name).length;
                const subCount = NoteService.getNoteSubFolders(folder.name).length;
                return { ...folder, hasContent: count > 0 || subCount > 0 };
            });

            UI.renderFolderGrid(folders, noteDisplay,
                (name) => { Folders.note.active = name; renderNotes(); },
                (folder, menu, closeMenu) => {
                    const renameBtn = document.createElement('button');
                    renameBtn.className = 'bm-button';
                    renameBtn.textContent = '✏️ Rename';
                    ...
                    moveBtn.className = 'bm-button';
                    moveBtn.textContent = '📂 Move';
                    moveBtn.onclick = () => { closeMenu(); showMoveModal('note-folder', folder.name, null); };
        ...
        if (!noteSearchQuery && Folders.note.active) {
            notes = notes.filter(note => note.folderId === Folders.note.active && (Folders.note.activeSub ? note.subfolderId === Folders.note.activeSub : !note.subfolderId));
        ...
        if (!noteSearchQuery && Folders.note.active) {
            const headerRow = document.createElement('div');
            Object.assign(headerRow.style, { display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: '8px', width: '100%', marginBottom: '2px' });
            const backBtn = document.createElement('button');
            backBtn.className = 'bm-button';
            backBtn.textContent = '⬅Back';
            Object.assign(backBtn.style, { marginBottom: '0', flex: '0 0 auto', visibility: 'visible' });
            ...
            const titleDiv = document.createElement('div');
            titleDiv.className = 'bm-header folder-name';
            titleDiv.textContent = Folders.note.activeSub ? `${Folders.note.active} / ${Folders.note.activeSub}` : Folders.note.active;
            Object.assign(titleDiv.style, {
                fontWeight: 'bold', fontSize: '13px', color: '#000', letterSpacing: '1px',
                textAlign: 'center', margin: '0', display: 'block', flex: '1 1 0%', minWidth: '0'
            });
            ...
            if (!Folders.note.activeSub) {
                const subs = NoteService.getNoteSubFolders(Folders.note.active).sort((a, b) => a.name.localeCompare(b.name));
                if (subs.length > 0) {
                    const subGrid = document.createElement('div');
                    Object.assign(subGrid.style, { display: 'flex', flexWrap: 'wrap', gap: '10px', width: '100%', marginBottom: '10px' });
                    subs.forEach(sf => {
                        const count = allNotes.filter(n => !n.archived && n.folderId === Folders.note.active && n.subfolderId === sf.name).length;
                        const wrapper = document.createElement('div');
                        wrapper.className = 'bm-folder-icon-wrapper';
                        wrapper.title = `${sf.name} (${count})`;
                        ...
                        wrapper.addEventListener('dblclick', () => {
                            Folders.note.activeSub = sf.name;
                            renderNotes();
                        });
                        ...
                        menuIcon.addEventListener('click', e => {
                            ...
                            moveBtn.className = 'bm-button';
                            moveBtn.textContent = '📂 Move';
                            moveBtn.onclick = () => { closeMenu(); showMoveModal('note-subfolder', sf.name, Folders.note.active); };

                            const delBtn = document.createElement('button');
                            delBtn.className = 'bm-button';
                            delBtn.textContent = '🗑️ Delete';
                            delBtn.onclick = () => { closeMenu(); bmConfirm(`Delete folder "${sf.name}" and ALL its notes?`, ok => { if (ok) { NoteService.deleteNoteSubFolder(Folders.note.active, sf.name); renderNotes(); } }); };

                            menu.append(moveBtn, delBtn);
                        });
                        wrapper.appendChild(menuIcon);
                        subGrid.appendChild(wrapper);
                    });
                    noteDisplay.appendChild(subGrid);
                }
            }
        }
        ...
        if (selectedNoteIds.size > 0 && notes.length > 0) {
            ...
            batchBar.append(selectAllBtn, moveBtn, deleteBtn);
    ...
    function showManageProjectsModal() {
        if (UI.panel.element.querySelector('.bm-modal')) return;
        ...
        const defaultProjectRow = document.createElement('div');
        defaultProjectRow.style.display = 'flex';
        defaultProjectRow.style.alignItems = 'center';
        defaultProjectRow.style.justifyContent = 'space-between';
        defaultProjectRow.style.marginTop = '4px';
        defaultProjectRow.style.paddingTop = '6px';
        defaultProjectRow.style.borderTop = '1px solid #eee';

        const defaultLabel = document.createElement('span');
        defaultLabel.textContent = 'Default Project:';
        defaultLabel.style.fontSize = '12px';
        defaultLabel.style.fontWeight = 'bold';
        defaultLabel.style.color = '#333';

        const defaultSelect = document.createElement('select');
        defaultSelect.className = 'bm-todo-select';
        defaultSelect.style.padding = '4px';
        defaultSelect.style.fontSize = '12px';

        const populateDefaultSelect = () => {
            defaultSelect.innerHTML = '';
            const projects = getTodoProjects();
            const currentDefault = getDefaultTodoProject();
            projects.forEach(p => {
                const opt = document.createElement('option');
                opt.value = p;
                opt.textContent = `📁 ${p}`;
                if (p === currentDefault) opt.selected = true;
                defaultSelect.appendChild(opt);
            });
        };

        defaultSelect.addEventListener('change', () => {
            setDefaultTodoProject(defaultSelect.value);
            showFlash(`Default project set to "${defaultSelect.value}"`, false, 'success');
            populateProjectSelects();
        });

        defaultProjectRow.append(defaultLabel, defaultSelect);
                ...
                renameBtn.addEventListener('click', (e) => {
                    ...
                    const saveHandler = (e2) => {
                        ...
                        if (ok) {
                            ...
                            if (gmGet('app:todo:default_project', '') === p) {
                                gmSet('app:todo:default_project', val);
                            }
                            renderProjectsListInModal();
                            populateProjectSelects();
                            renderTasksList();
        ...
        body.appendChild(defaultProjectRow);
    ...
    projectSelect.addEventListener('change', () => {
        if (projectSelect.value === '__manage__') {
            ...
            // Revert selection back to the default project
            projectSelect.value = getDefaultTodoProject();
        }
    });
    ...
    function populateMainProjectSelect() {
        ...
        const projects = getTodoProjects();
        const defaultProject = getDefaultTodoProject();
        projects.forEach(p => {
            const opt = document.createElement('option');
            opt.value = p;
            opt.textContent = `📁 ${p}`;
            if (p === defaultProject) opt.selected = true;
    ...
    UI.registerTab('todo', '📋 Todo', todoTabContent);
                ...
                if (!globalTimerNotified) {
                    ...
                    if (typeof sendAppNotification === 'function') {
                        sendAppNotification({
                            ...
                            body: 'Your selected time is up.',
                            tag: 'pa-timer',
                            fallbackType: 'info',
                            onSnooze: () => {
                                const settings = getWellnessSettings();
                                globalTimerEndMs = Date.now() + settings.snoozeMinutes * 60000;
                                localStorage.setItem('PA_GLOBAL_TIMER', globalTimerEndMs);
                                globalTimerNotified = false;
                                if (typeof updateReminderCountdownDisplays === 'function') updateReminderCountdownDisplays();
                                showFlash(`Timer snoozed for ${settings.snoozeMinutes}m`, false, 'info');
                            }
                        });
                    }
    ...
    function showTaskDetailsModal(task) {
        ...
        if (!task.completed && !task.archived && (task.reminderTime || task.dueDate)) {
            ...
            snoozeBtn.title = `Snooze reminder ${settings.snoozeMinutes} minutes`;
            styleIconModalButton(snoozeBtn);
            snoozeBtn.addEventListener('click', () => {
                const ok = snoozeTaskReminder(task.id, settings.snoozeMinutes);
                if (!ok) return;
                modal.remove();
                renderTasksList();
                showFlash(`Reminder snoozed for ${settings.snoozeMinutes} minutes`, false, 'success');
            });
    ...
    /* Folders.barcode.active moved to Folders */
    /* Folders.barcode.activeSub moved to Folders */
    ...
    // above, then restore the UI.panel.element list view.
    // ============================================================

    function showFolderForm() {
        closeOpenFolderMenu();
        // Hide folderDisplay, show UI.formWrapper.element
        folderDisplay.style.display = 'none';
        UI.formWrapper.element.style.display = 'flex';
        ...
        UI.formWrapper.element.innerHTML = '';
        UI.formWrapper.element.appendChild(form);

        function restoreDisplayAndLayout() {
            UI.formWrapper.element.innerHTML = '';
            UI.formWrapper.element.style.display = 'none'; // hide UI.formWrapper.element after done
            folderDisplay.style.display = 'flex';
    ...
    async function showUniversalMoveModal(options) {
        const {
            title,
            itemType, // 'file' (or batch files), 'subfolder', 'folder'
            itemName, // name of folder/subfolder if applicable (to prevent moving into itself)
            currentParent, // the parent of the subfolder being moved (if applicable)
            hasSubfolders, // true if itemType is 'folder' and it contains subfolders
            folders, // array of {name}
            subfolders, // array of {name, parent}
            onApply,
            onCancel
        } = options;

        const existing = document.getElementById('bm-universal-move-modal');
        if (existing) existing.remove();

        const modal = document.createElement('div');
        modal.id = 'bm-universal-move-modal';
        modal.className = 'bm-modal';
        modal.dataset.noAutoclose = '1';
        Object.assign(modal.style, {
            padding: '16px',
        ...
        folders.forEach(f => {
            if (itemType === 'folder' && f.name === itemName) return;
            ...
            // Always show subfolders for full structure, but disable them if itemType isn't a file
            const sfs = subfolders.filter(sf => sf.parent === f.name);
            sfs.forEach(sf => {
                // If we are moving a subfolder, prevent moving it into itself
                if (itemType === 'subfolder' && sf.name === itemName && sf.parent === currentParent) return;

                const sfOpt = document.createElement('option');
                sfOpt.value = `${f.name}|${sf.name}`;
                sfOpt.innerHTML = `&nbsp;&nbsp;↳ 📁 ${sf.name}`;
                if (itemType !== 'file') {
                    sfOpt.disabled = true;
                }
                destSelect.appendChild(sfOpt);
            });
        });
        ...
        if (itemType === 'subfolder') {
            ...
            destSelect.value = destSelect.options[0].value;
        }
        ...
        applyBtn.addEventListener('click', async () => {
            let destFolder = '';
            let destSub = '';
            if (destSelect.value !== '__ROOT__') {
                const parts = destSelect.value.split('|');
                destFolder = parts[0];
                if (parts.length > 1) {
                    destSub = parts[1];
                }
            ...
            modal.remove();
            await onApply(destFolder, destSub);
        });
    ...
    async function showMoveModal(itemType, itemName, currentParent) {
        let getFoldersFn, getSubFoldersFn, hasSubFn;
        let baseType = itemType;
        let title = '';

        if (itemType.startsWith('note-')) {
            getFoldersFn = NoteService.getNoteFolders;
            getSubFoldersFn = NoteService.getAllNoteSubFolders;
            baseType = itemType.replace('note-', '');
            hasSubFn = (name) => NoteService.getAllNoteSubFolders().some(sf => sf.parent === name);
            title = baseType === 'folder' ? `Move Note Folder (${itemName})` : `Move Note Subfolder (${itemName})`;
        } else {
            getFoldersFn = getFolders;
            getSubFoldersFn = getAllSubFolders;
            hasSubFn = (name) => getAllSubFolders().some(sf => sf.parent === name);
            title = baseType === 'folder' ? `Move Folder (${itemName})` : `Move Subfolder (${itemName})`;
        }

        const folders = await getFoldersFn();
        const subfolders = getSubFoldersFn();
        const hasSubfolders = baseType === 'folder' && hasSubFn(itemName);

        showUniversalMoveModal({
            title,
            itemType: baseType,
            itemName,
            currentParent, // Pass this down so subfolders can be correctly filtered
            hasSubfolders,
            folders,
            subfolders,
            onApply: async (destFolder, destSub) => {
                if (itemType === 'folder') await moveFolderTo(itemName, destFolder);
                else if (itemType === 'subfolder') await moveSubFolderTo(currentParent, itemName, destFolder);
                else if (itemType === 'note-folder') await NoteService.moveNoteFolderTo(itemName, destFolder);
                else if (itemType === 'note-subfolder') await NoteService.moveNoteSubFolderTo(currentParent, itemName, destFolder);
            }
        });
    }

    // Move a batch of selected barcodes to a chosen folder / nested folder.
    async function showMoveBarcodesModal(ids, onDone) {
        const folders = await getFolders();
        const subfolders = getAllSubFolders();

        showUniversalMoveModal({
            title: `Move ${ids.length} barcode(s) to`,
            itemType: 'file',
            folders,
            subfolders,
            onApply: async (destFolder, destSub) => {
                await moveBarcodesToFolder(ids, destFolder || 'Default', destSub);
                if (typeof onDone === 'function') onDone(destFolder || 'Default', destSub);
            }
        });
    }
    ...
    function showBarcodeForm(barcodeToEdit = null) {
        ...
        folderDisplay.style.display = 'none';
        UI.formWrapper.element.style.display = 'flex';
        UI.formWrapper.element.style.flex = '1 1 auto';
        UI.formWrapper.element.style.overflow = 'auto';
        // Hide footerLeft (item count) during barcode creation/edit
        footerLeft.style.visibility = 'hidden';

        // --- Dynamically adjust UI.panel.element height for barcode form ---
        const prevPanelHeight = UI.panel.element.style.height;
        UI.panel.element.style.height = 'auto';
        ...
        // --- Combined Folder / Folder Tree Select ---
        const initFolder = barcodeToEdit ? (barcodeToEdit.folder || Folders.barcode.active || 'Default') : (Folders.barcode.active || 'Default');
        const initSub = barcodeToEdit ? (barcodeToEdit.subfolder || '') : (Folders.barcode.activeSub || '');
        ...
        function getPreviewData(rawValue, rawFormat) {
            const data = getNormalizedBarcodeData(rawValue, rawFormat);
            if (!data.jsbValue) return null;
            return data;
        }
        ...
        function adjustBarcodeFormPanelHeight() {
            setTimeout(() => {
                const footerElem = UI.panel.element.querySelector('.bm-footer');
                const footerHeight = footerElem ? footerElem.offsetHeight : 0;
                const formHeight = Math.max(UI.formWrapper.element.scrollHeight || 0, UI.formWrapper.element.offsetHeight || 0, form.scrollHeight || 0);
                ...
                UI.panel.element.style.height = Math.min(Math.max(previousHeight, neededHeight, 360), maxPanelHeight) + 'px';
            }, 0);
        }
        ...
        function restoreDisplayAndLayout() {
            ...
            // Restore UI.panel.element height to default
    ...
    function showImportModal() {
        ...
        const createFolderSelect = (preferred) => {
            return createFolderDestinationSelect(preferred || 'Default', Folders.barcode.activeSub || '', {
                fontSize: '12px',
                padding: '4px 6px',
                height: '28px',
                flex: '1',
        ...
        jsonDesc.textContent = 'Use the Export File output (.zip or .pa). Includes all your workspace data.';
        const jsonBtn = document.createElement('button');
        jsonBtn.className = 'bm-button';
        jsonBtn.textContent = 'Choose File';
        jsonBtn.addEventListener('click', () => {
            pickFileAsArrayBuffer('.pa, .zip', async (buffer, file) => {
                try {
                    // 1. Try backwards compatibility for plain JSON .pa files (Workspace Snapshots)
                    try {
                        const text = new TextDecoder().decode(buffer);
                        if (text.trim().startsWith('{')) {
                            const jsonData = JSON.parse(text);
                            if (jsonData.schema === 'PA-backup' || jsonData.format === 'PA' || jsonData.folders) {
                                const payload = normalizeBackupPayload(jsonData);
                                const filename = file?.name || 'Selected file';
                                showPreview('PA Workspace (Legacy JSON)', filename, { kind: 'json', payload });
                                return;
                            }
                        }
                    } catch (e) {
                        // Not a valid JSON, fallback to JSZip
                    }

                    // 2. Process as JSZip archive
                    ...
                    // 3. Find the workspace data file
                    let workspaceFile = zip.file("workspace.pa") || zip.file("workspace.json");

                    // (Optional fallback for the temporary double-zip architecture)
                    if (!workspaceFile) {
                        const paFiles = Object.keys(zip.files).filter(name => name.endsWith('.pa') && !zip.files[name].dir);
                        if (paFiles.length === 1) {
                            const paBuffer = await zip.file(paFiles[0]).async("arraybuffer");
                            const innerZip = await JSZip.loadAsync(paBuffer);
                            workspaceFile = innerZip.file("workspace.json");
                        }
                    }

                    if (!workspaceFile) throw new Error("Missing workspace data file");
        ...
        const csvTxtFolderSelect = createFolderSelect(Folders.barcode.active || 'Default');
    ...
    function showRenameModal(oldName, callback) {
        ...
        header.style.textAlign = 'center';
        header.style.margin = '4px 0 8px 0';
        UI.panel.element.appendChild(header);
    ...
    async function renderFolders(options = {}) {
        ...
        const isFormOpen = UI.formWrapper.element.style.display !== 'none' && UI.formWrapper.element.innerHTML !== '';
        if (options.backgroundSync && isFormOpen) {
            return; // Don't disrupt active barcode editing
        }
        folderDisplay.style.display = 'flex';
        UI.formWrapper.element.style.display = 'none'; // always hide UI.formWrapper.element unless creating/editing
        folderDisplay.innerHTML = '';
        let folderMenu = null;
        let barcodeMenu = null;

        const inFolder = !!Folders.barcode.active;
        const inSubFolder = !!Folders.barcode.activeSub;

        const toolbar = document.createElement('div');
        Object.assign(toolbar.style, {
            display: 'flex', alignItems: 'center', gap: '6px', width: '100%', flexWrap: 'wrap', marginBottom: '10px'
        });
        const newBtn = UI.createToolbarButton('New Barcode', '🏷️', () => showBarcodeForm());
        toolbar.appendChild(newBtn);

        const newFolderBtn = UI.createToolbarButton('New folder', '📁', () => {
            showNewFolderModal((name) => {
                if (name !== null) {
                    if (inFolder) saveSubFolder(Folders.barcode.active, name);
                    else saveFolder(name);
                }
            });
        });
        if (!inSubFolder) {
        ...
        folderDisplay.appendChild(toolbar);

        if (Folders.barcode.active) {
            // --- Folder Header Row ---
            const folderHeaderRow = document.createElement('div');
            Object.assign(folderHeaderRow.style, {
                display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
                gap: '8px', marginBottom: '10px', width: '100%'
            });
            ...
            folderNameDiv.textContent = inSubFolder ? `${Folders.barcode.active} / ${Folders.barcode.activeSub}` : Folders.barcode.active;
            ...
            folderHeaderRow.appendChild(placeholder);

            // Remove any previous folder name header
            const prevFolderName = UI.panel.element.querySelector('.bm-header.folder-name');
            if (prevFolderName) prevFolderName.remove();

            // --- Folder cards (only at folder level) ---
            ...
            if (!inSubFolder) {
                ...
                const folderSubFolders = getSubFolders(Folders.barcode.active);
                ...
                const subFolderBarcodes = await idbGetBarcodesByFolder(Folders.barcode.active);
                if (isStale()) return;
                ...
                folderSubFolders.forEach(sf => {
                    ...
                    sfIconWrapper.addEventListener('dblclick', () => {
                        Folders.barcode.activeSub = sf.name;
                        renderFolders();
                    });

                    const sfMenuIcon = document.createElement('div');
                    UI.styleFolderMenuIcon(sfMenuIcon, true);
                    ...
                    sfMenuIcon.addEventListener('click', (e) => {
                        ...
                        renameSfBtn.addEventListener('click', () => {
                            ...
                            showRenameModal(sf.name, (newName) => {
                                if (newName && newName.trim() !== '' && newName.toLowerCase() !== sf.name.toLowerCase()) {
                                    renameSubFolder(Folders.barcode.active, sf.name, newName);
                        ...
                        printSfBtn.addEventListener('click', () => {
                            closeSfMenu();
                            (async () => {
                                const sfBarcodes = await idbGetBarcodesByFolder(Folders.barcode.active, sf.name);
                        ...
                        formatSfBtn.addEventListener('click', () => {
                            closeSfMenu();
                            showFolderChangeFormatModal(Folders.barcode.active, sf.name);
                        });
                        ...
                        moveSfBtn.addEventListener('click', () => {
                            closeSfMenu();
                            showMoveModal('subfolder', sf.name, Folders.barcode.active);
                        });
                        ...
                        deleteSfBtn.addEventListener('click', () => {
                            closeSfMenu();
                            bmConfirm(`Delete folder "${sf.name}" and all its barcodes?`, (result) => {
                                if (result) deleteSubFolder(Folders.barcode.active, sf.name);
                            });
                        });
                        ...
                        pinSfBtn.addEventListener('click', () => {
                            updateSubFolder(Folders.barcode.active, sf.name, { pinned: !sf.pinned });
                            showFlash(sf.pinned ? 'Folder unpinned' : 'Folder pinned', false, 'success');
                            closeSfMenu();
                        });
            ...
            const barcodes = await idbGetBarcodesByFolder(Folders.barcode.active, Folders.barcode.activeSub || '');
            if (isStale()) return;
            ...
            if (!window._barcodeFlashActive) {
                const folderSubCount = inSubFolder ? 0 : getSubFolders(Folders.barcode.active).length;
                if (barcodes.length === 0 && folderSubCount === 0) {
                    footerCenter.dataset.bmSystemMessage = '1';
                    const emptyMsg = inSubFolder ? 'This nested folder is empty' : 'This folder is empty';
            ...
            const createBarcodeItem = (barcode) => {
                ...
                if (barcode.format === 'TEXT') {
                    ...
                    try {
                        const { jsbFormat: normFormat, jsbValue: normValue } = getNormalizedBarcodeData(barcode.value, barcode.format);
                        let jsbFormat = normFormat;
        ...
        // Toolbar is now rendered at the top of the function
        ...
        folders.forEach(folder => {
            ...
            // A folder also counts as non-empty when it contains nested folders.
    ...
    function showBigBarcodeModal(value, format, name, options = {}) {
        ...
        const renderLinearCanvas = (jsbFormat, holderWidthPx, holderHeightPx, opts = {}) => {
            ...
            const { jsbFormat: normFormat, jsbValue: normValue } = getNormalizedBarcodeData(value, jsbFormat);
            const valueLen = Math.max(1, normValue.length);
            const barWidth = computeBarWidth(normFormat, valueLen, displayWidth, dpr);
            const modules = estimateModules(normFormat, valueLen);
        ...
        const renderLinearLabelPreview = (jsbFormat, renderBoxEl, opts = {}) => {
            ...
            try {
                ...
                const { jsbFormat: normFormat, jsbValue: normValue } = getNormalizedBarcodeData(value, jsbFormat);
                const valueLen = Math.max(1, normValue.length);
                const barWidth = computeBarWidth(normFormat, valueLen, codeAreaWidthPx, 1);
                const modules = estimateModules(normFormat, valueLen);
                const svgWidthPx = Math.round(modules * barWidth);
                JsBarcode(svg, normValue, {
                    format: normFormat,
                    lineColor: '#000',
                    width: barWidth,
            ...
            const { jsbValue: normValue } = getNormalizedBarcodeData(value, jsbFormat);
            const valueLine = document.createElement('div');
            valueLine.textContent = String(normValue ?? '');
        ...
        if (options?.fromShortcut) {
            ...
            quickFolderSelect = createFolderDestinationSelect(Folders.barcode.active || 'Default', Folders.barcode.activeSub || '', {
                fontSize: '12px',
                padding: '4px 6px',
                height: '28px',
                minWidth: '90px',
            ...
            quickFolderSelect.addEventListener('change', () => {
                ...
                const lastValid = quickFolderSelect.dataset.lastValid || (Folders.barcode.active || 'Default');
        ...
        if (options?.fromShortcut) {
            ...
            saveBtn.addEventListener('click', () => runActionWithFeedback(saveBtn, async () => {
                ...
                UI.wireModalIdleTracking(saveModal);
    ...
    async function initialize() {
        ...
        await renderFolders();
        document.body.appendChild(UI.panel.element);
        floatingButton.addEventListener('click', UI.togglePanel);
        // Keep UI.panel.element closed by default
        UI.panel.hide();
    ...
    // final initialize() call. They close over UI.panel.element/footer state.
    // ============================================================
    ...
    // Add dropdown to UI.panel.element
    UI.panel.element.appendChild(dropdown);
    ...
    aboutBtn.onclick = function () {
        ...
        if (aboutModal && UI.panel.element.contains(aboutModal)) {
            aboutModal.remove();
            aboutModal = null;
            return;
        }
        ...
        aboutModal.innerHTML = `
            ...
            `;
        UI.panel.element.appendChild(aboutModal);
        UI.wireModalIdleTracking(aboutModal);
        ...
        function escHandler(e) {
            if (e.key === 'Escape' && aboutModal && UI.panel.element.contains(aboutModal)) {
                aboutModal.remove();
                aboutModal = null;
                document.removeEventListener('keydown', escHandler);
            }
    ...
    // Add spacer and footer to UI.panel.element (always as last children)
    if (UI.panel.element.lastChild !== footerSpacer) {
        UI.panel.element.appendChild(footerSpacer);
    }
    if (UI.panel.element.lastChild !== footer) {
        UI.panel.element.appendChild(footer);
    }
    ...
    async function updateFooterCount() {
        ...
        if (currentTabName === 'bookmarks') {
            const bookmarks = getBookmarks();
            if (Folders.bookmark.activeSub) {
                const count = bookmarks.filter(b => b.folder === Folders.bookmark.active && b.subfolder === Folders.bookmark.activeSub).length;
                setFooterLeftText(`Bookmarks: ${count}`);
            } else if (Folders.bookmark.active) {
                const subFolders = getBookmarkSubFolders(Folders.bookmark.active);
                const directBookmarks = bookmarks.filter(b => b.folder === Folders.bookmark.active && !b.subfolder).length;
                setFooterLeftText(`Nested folders: ${subFolders.length} · Bookmarks: ${directBookmarks}`);
            } else {
                const folders = getBookmarkFolders();
        ...
        if (currentTabName === 'notes') {
            ...
            } else if (Folders.note.active) {
                const folderNotes = notes.filter(note => !note.archived && note.folderId === Folders.note.active).length;
                setFooterLeftText(`${Folders.note.active}: ${folderNotes}`);
            } else {
        ...
        } else if (Folders.barcode.active) {
            const subFolders = getSubFolders(Folders.barcode.active);
            const directBarcodes = await idbGetBarcodesByFolder(Folders.barcode.active, '');
            setFooterLeftText(`Nested folders: ${subFolders.length} · Barcodes: ${directBarcodes.length}`);
        } else {
            const folders = await getFolders();
            setFooterLeftText(`Folders: ${folders.length}`);
        }
    ...
    const oldSwitchTabForFooter = UI.switchTab;
    UI.switchTab = function (tabName) {
    ...
    GM_addStyle(`
        ...
        /* Ensure folderDisplay and UI.formWrapper.element scrollbars are on the right */
        .bm-panel > div,
        ...
        .bm-note-editor img.bm-note-img-selected {
        ...
        /* PA Modern Settings Panel Styles */
        .pa-status-dot {
            display: inline-block !important;
            width: 8px !important;
            height: 8px !important;
        ...
        .pa-action-btn, .pa-btn-group button {
        ...
        .pa-action-btn:hover, .pa-btn-group button:hover { 
            background: #e2e8f0 !important;
            border-color: #94a3b8 !important;
        }

        .pa-action-btn:disabled, .pa-btn-group button:disabled {
